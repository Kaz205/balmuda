...
{
	int i = 0;
	struct ft_chip_t ctype[] = {{0x81, 0x54, 0x52, 0x54, 0x52, 0x00,
					0x00, 0x54, 0x5C}};
	u32 ctype_entries = ARRAY_SIZE(ctype);
...
static int fts_input_report_b(struct fts_ts_data *data)
{
	...
	for (i = 0; i < data->touch_point; i++) {
		...
		if (EVENT_DOWN(events[i].flag)) {
			...
			if ((data->log_level >= 2) || ((1 == data->log_level)
...
static int fts_power_source_ctrl(struct fts_ts_data *ts_data, int enable)
{
	...
	if (enable) {
		if (ts_data->power_disabled) {
			...
			/* Add */
			gpio_direction_output(ts_data->pdata->vtp30_gpio, 1);
			msleep(5); /*TEST*/
			/* Add */

			ret = regulator_enable(ts_data->vdd);
			if (ret) {
				FTS_ERROR("enable vdd regulator failed");
			}

			if (!IS_ERR_OR_NULL(ts_data->vcc_i2c)) {
				ret = regulator_enable(ts_data->vcc_i2c);
				if (ret) {
					FTS_ERROR("enable vcc_i2c failed");
				}
			}
		...
		if (!ts_data->power_disabled) {
			...
			/* Add */
			gpio_direction_output(ts_data->pdata->vtp30_gpio, 0);
			/* Add */

			ret = regulator_disable(ts_data->vdd);
			if (ret) {
				FTS_ERROR("disable vdd regulator failed");
			}
			if (!IS_ERR_OR_NULL(ts_data->vcc_i2c)) {
				ret = regulator_disable(ts_data->vcc_i2c);
				if (ret) {
					FTS_ERROR("disable vcc_i2c failed");
				}
			}
	...
	if (!IS_ERR_OR_NULL(ts_data->vcc_i2c)) {
		if (regulator_count_voltages(ts_data->vcc_i2c) > 0) {
			ret = regulator_set_voltage(ts_data->vcc_i2c,
						...
						FTS_I2C_VTG_MAX_UV);
			if (ret) {
				FTS_ERROR("vcc_i2c set_vtg failed,ret=%d", ret);
...
static int fts_gpio_configure(struct fts_ts_data *data)
{
	...
	/* request VTP(v3.0) gpio */
	if (gpio_is_valid(data->pdata->vtp30_gpio)) {
//		ret = gpio_request(data->pdata->vtp30_gpio, "fts_vtp30_gpio");
//		if (ret) {
//			FTS_ERROR("[GPIO]vtp30 gpio request failed");
//			goto err_reset_gpio_dir;
//		}
		...
		if (ret) {
			FTS_ERROR("[GPIO]set_direction for reset gpio failed");
			goto err_vtp30_gpio_dir;
		}
	}
#if 0
	/* request VTP(v1.8) gpio */
	if (gpio_is_valid(data->pdata->reset_gpio)) {
		ret = gpio_request(data->pdata->reset_gpio, "fts_reset_gpio");
		if (ret) {
			FTS_ERROR("[GPIO]reset gpio request failed");
...
err_vtp30_gpio_dir:
	if (gpio_is_valid(data->pdata->vtp30_gpio))
		gpio_free(data->pdata->vtp30_gpio);
err_reset_gpio_dir:
	if (gpio_is_valid(data->pdata->reset_gpio))
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
	...
	/* Add */
	pdata->vtp30_gpio = of_get_named_gpio_flags(np, "kc,vtp30-gpio",
			0, &pdata->vtp30_gpio_flags);
	if (!gpio_is_valid(pdata->vtp30_gpio))
		FTS_ERROR("Unable to get vtp30_gpio");

	pdata->vtp18_gpio = of_get_named_gpio_flags(np, "kc,vtp18-gpio",
			0, &pdata->vtp18_gpio_flags);
	if (!gpio_is_valid(pdata->vtp18_gpio))
		FTS_ERROR("Unable to get vtp18_gpio");
	/* Add */
	...
	FTS_INFO("vtp30 gpio:%d, vtp18 gpio:%d",
		pdata->vtp30_gpio, pdata->vtp18_gpio);
...
err_power_init:
	...
	if (gpio_is_valid(ts_data->pdata->vtp30_gpio))
		gpio_free(ts_data->pdata->vtp30_gpio);
	if (gpio_is_valid(ts_data->pdata->vtp18_gpio))
		gpio_free(ts_data->pdata->vtp18_gpio);
