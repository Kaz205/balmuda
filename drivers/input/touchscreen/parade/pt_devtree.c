...
struct drm_panel *pt_active_panel = NULL;
		...
		struct device_node *core_node)
{
	...
	rc = of_property_read_u32(core_node, "kc,glove_valid", &value);
	if (!rc) {
		pdata->glove_valid = (u8)value;
		if (value)
			pr_err("%s: Glove Valid\n", __func__);
		else
			pr_err("%s: Glove Invalid\n", __func__);
	}

	rc = of_property_read_u32(core_node, "kc,easywake_valid", &value);
	if (!rc) {
		pdata->easywake_valid = (u8)value;
		if (value)
			pr_err("%s: Easywake Valid\n", __func__);
		else
			pr_err("%s: Easywake Invalid\n", __func__);
	}

	rc = of_property_read_u32(core_node, "kc,fw_type", &value);
	if (!rc) {
		pdata->fw_type = (u8)value;
		pr_err("%s: FW Type Valid (value:%d)\n", __func__, value);
	}else{
		pdata->fw_type = 0;
		pr_err("%s: FW Type Invalid\n", __func__);
	}
	pr_err("%s: FW Type=%d\n", __func__, pdata->fw_type);
	...
	count = of_count_phandle_with_args(core_node, "panel", NULL);
	for (i = 0; i < count; i++) {
		node = of_parse_phandle(core_node, "panel", i);
		panel = of_drm_find_panel(node);
		of_node_put(node);
		if (!IS_ERR(panel)) {
			pt_active_panel = panel;
			pr_err("Got active panel\n");
		}
	}

	pr_info("%s: irq_gpio:%d rst_gpio:%d\n"
