...
/*** kctouch ***/
#define PT_GLOVE_OFF		0
#define PT_GLOVE_ON			1
#define PT_GLOVE_AUTO		2
...
#define PT_WATCHDOG_TIMEOUT                     5000
...
#define PIP1_CMD_GET_SYSINFO_TIMEOUT             500
...
#define KC_TOUCH_EVENT_RESTART          300
#define KC_TOUCH_EVENT_STOP             301
#define KC_TOUCH_FACTORY_LOCK           302
#define KC_TOUCH_FACTORY_UNLOCK         303
#define KC_TOUCH_POWER_OFF              304
#define KC_TOUCH_POWER_ON               305
...
#define PT_MAX_AREA_SIZE        (20)
#define PT_MAX_PRESSURE_SIZE    (250)
...
#define PT_RAM_ID_TOUCHMODE_ENABLED			0x02
#define PT_RAM_ID_GSM_TOUCHMODE_ENABLED		0x30

enum pt_touchmode_enabled_mode {
	PT_TME_NULL				= 0,
	PT_TME_FINGER_ONLY		= 1,
	PT_TME_FINGER_AND_GLOVE	= 3,
};
...
enum PT_REQUEST_BIN_HDR_RESULT {
	PT_BIN_HDR_PASS              = 0,
	PT_BIN_HDR_SIZE_ERROR        = 1,
	PT_BIN_HDR_LEN_ERROR         = 2,
	PT_BIN_HDR_FILE_OPEN_FAIL    = 3,
	PT_BIN_HDR_FILE_CLOSE_FAIL   = 4,
	PT_BIN_HDR_FW_NOT_EXIST      = 5,
	PT_BIN_HDR_COMM_ERROR        = 6,
};
...
struct pt_mt_data {
	...
	int t_max;
#ifdef CONFIG_TOUCHSCREEN_KC_TOUCHMODE_ENABLED
	bool check_finger_touch;
	bool check_large_touch;
	bool request_glove_touch;
#endif /* CONFIG_TOUCHSCREEN_KC_TOUCHMODE_ENABLED */
};
...
struct pt_core_commands {
	...
	int (*request_pip2_launch_app)(struct device *dev, u8 major, u8 minor,
		int protect);
...
#ifdef CONFIG_TOUCHSCREEN_KC_TOUCHMODE_ENABLED
	int (*set_touchmode_enabled)(struct device *dev, u8 mode);
#endif /* CONFIG_TOUCHSCREEN_KC_TOUCHMODE_ENABLED */
};
...
#define PT_PREALLOCATED_CMD_BUFFER 32
	...
	struct kc_touch_info touch_info[KC_TS_MAX_FINGER];
	int touch_cnt;
	int ts_event_control;
	bool startup_flg;
	bool ldr_exit_flg;
	bool fw_loading_flg;
	...
	atomic_t loader_finished;
#ifdef CONFIG_TOUCHSCREEN_KC_TOUCHMODE_ENABLED
	struct work_struct touchmode_work;
	atomic_t touchmode_enabled;
#endif /* CONFIG_TOUCHSCREEN_KC_TOUCHMODE_ENABLED */
#ifdef CONFIG_TOUCHSCREEN_KC_TOUCHMODE_IOCTL
	atomic_t force_finger;
#endif /* CONFIG_TOUCHSCREEN_KC_TOUCHMODE_IOCTL */
#ifdef CONFIG_INCELL_TOUCH
	struct completion first_suspend_done;
	bool first_suspend;
#else
	struct completion fw_update_done;
	bool factory_state;
#endif
	struct completion exit_bootmode;
	struct mutex factory_lock;
	struct mutex dut_start_lock;
	struct mutex kcsysfs_lock;
	int	restart_count;
	struct work_struct restart_work;
	struct work_struct kc_resume_work;
};
