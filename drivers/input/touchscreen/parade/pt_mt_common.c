		...
		struct pt_touch *tch, int num_cur_tch)
{
	...
	for (i = 0; i < num_cur_tch; i++) {
		...
		if(tch->abs[PT_TCH_P] > PT_MAX_PRESSURE_SIZE)
			tch->abs[PT_TCH_P] = PT_MAX_PRESSURE_SIZE;
		...
		for (j = 0; j < PT_NUM_EXT_TCH_FIELDS; j++) {
			if (!si->tch_abs[PT_ABS_MAJ_OST + j].report)
				continue;
			if(tch->abs[PT_TCH_MAJ + j] < PT_MAX_AREA_SIZE) {
		...
		struct pt_mt_data *md)
{
	...
	input_report_abs(md->input, ABS_MT_POSITION_X, 1080);
	input_report_abs(md->input, ABS_MT_POSITION_Y, 2160);
	input_report_abs(md->input, ABS_MT_PRESSURE, 62);
	input_report_abs(md->input, ABS_MT_TOUCH_MAJOR, 19);
	input_report_abs(md->input, ABS_MT_TOUCH_MINOR, 19);
	input_report_abs(md->input, ABS_MT_ORIENTATION, 0);
...
#ifdef CONFIG_TOUCHSCREEN_KC_TOUCHMODE_ENABLED
static int pt_mt_glove_attention(struct device *dev)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	struct pt_mt_data *md = &cd->md;
	struct pt_core_commands *cmd = pt_get_commands();

	cmd->set_touchmode_enabled(dev, PT_TME_FINGER_AND_GLOVE);
...
static int pt_mt_finger_attention(struct device *dev)
{
	struct pt_core_commands *cmd = pt_get_commands();

	cmd->set_touchmode_enabled(dev, PT_TME_FINGER_ONLY);
...
#ifdef CONFIG_TOUCHSCREEN_KC_TOUCHMODE_ENABLED
	_pt_subscribe_attention(dev, PT_ATTEN_GLOVE, PT_MT_NAME,
		pt_mt_glove_attention, 0);

	_pt_subscribe_attention(dev, PT_ATTEN_FINGER, PT_MT_NAME,
		pt_mt_finger_attention, 0);
#endif /* CONFIG_TOUCHSCREEN_KC_TOUCHMODE_ENABLED */
...
#ifdef CONFIG_TOUCHSCREEN_KC_TOUCHMODE_ENABLED
	_pt_unsubscribe_attention(dev, PT_ATTEN_GLOVE, PT_MT_NAME,
		pt_mt_glove_attention, 0);

	_pt_unsubscribe_attention(dev, PT_ATTEN_FINGER, PT_MT_NAME,
		pt_mt_finger_attention, 0);
#endif /* CONFIG_TOUCHSCREEN_KC_TOUCHMODE_ENABLED */
