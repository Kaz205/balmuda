...
extern int kctouch_startup_state;
...
#if defined(PT_FW_UPGRADE) || defined(PT_TTCONFIG_UPGRADE)
/*******************************************************************************
...
static int kctp_check_firmware_type(struct device *dev,
	const struct firmware *fw, struct pt_bin_file_hdr *hdr)
{
	...
	u32 fw_type;
	int upgrade = 0;
	u16 fw_config_ver = ld->si->ttdata.fw_ver_conf;

	/* Offset 17,18 is the TT Config version*/
	fw_config_ver_new = get_unaligned_be16(fw->data + 17);
	fw_type = cd->cpdata->fw_type;

	pt_debug(dev, DL_WARN,
		 "%s: Current config ver:0x%04X New config ver:0x%04X\n",
		 __func__, fw_config_ver, fw_config_ver_new);

	if(fw_type == 3){
		if((fw_config_ver >> 8) != 0x65){
			pr_err("%s mismatch ver_conf = %x\n",__func__, (fw_config_ver>>8));
			upgrade=1;
		}
	}else{
		if((fw_config_ver >> 8) != 0x03){
			pr_err("%s mismatch ver_conf = %x\n",__func__, (fw_config_ver>>8));
			upgrade=1;
		}
	}
...
static char *generate_silicon_id_firmware_filename(struct device *dev)
{
	...
	pt_debug(dev, DL_INFO, "%s: FW Type=%d\n", __func__, fw_type);
	...
	if (panel_id == PANEL_ID_NOT_ENABLED)
		snprintf(filename, FILENAME_LEN_MAX, "%s_%02X%s", PT_FW_FILE_PREFIX, fw_type, PT_FW_FILE_SUFFIX);
	else
		snprintf(filename, FILENAME_LEN_MAX, "%s_%02X_pid%02X%s",
			PT_FW_FILE_PREFIX, fw_type, panel_id, PT_FW_FILE_SUFFIX);
...
#ifndef TTDL_KERNEL_SUBMISSION
#if defined(PT_TTCONFIG_UPGRADE)
/*******************************************************************************
...
static DEVICE_ATTR(config_loading, S_IWUSR,
	...
	const struct firmware *fw)
{
	...
	ret = kctp_check_firmware_type(dev, fw, &hdr);
	if (ret){
		pt_debug(dev, DL_WARN,
			"FW type is mismatch, will upgrade FW");
		should_upgrade = 1;
		goto exit;
	}
		...
		void *context)
{
	...
	if (!cd->bl_pip_ver_ready) {
		ret = cmd->request_active_pip_prot(dev, PT_CORE_CMD_UNPROTECTED,
			&pip_ver_major, &pip_ver_minor);
		if (ret) {
			pt_debug(dev, DL_ERROR,
				"%s: Failed to get PIP Version\n",
				__func__);
			_pt_pip2_update_bl_status(dev,
				UPDATE_FW_PIP_VERSION_ERROR, PT_NO_INC);
			goto exit;
		}
		...
		pip_ver_major = cd->bl_pip_version_major;
		pip_ver_minor = cd->bl_pip_version_minor;
	}
	...
	if (ld->pip2_load_builtin) {
		if (_pt_pip2_need_upgrade_due_to_fw_ver(dev, fw) ||
			...
			pr_notice("[KCTOUCH]%s: startup complete clear\n", __func__);
			kctouch_startup_state = 0;
		} else {
			_pt_pip2_update_bl_status(dev,
	...
	if ((ld->pip2_load_file_no == PIP2_RAM_FILE) &&
		...
		if (ret || status) {
		   ...
		   update_fw_status < UPDATE_FW_COMPLETE) {
		/*
		 * DUTs that do not support the RUNFW pin also don't support
		 * the EXIT_HOST_MODE ROM command and so a reset must be done
		 */
		if (pip_ver_major >= 2 && pip_ver_minor >= 3) {
			pt_debug(dev, DL_INFO,
				"%s Toggle TP_XRES now...\n", __func__);
			cmd->request_reset(dev, PT_CORE_CMD_UNPROTECTED);
		} else {
			cd->startup_status = STARTUP_STATUS_START;
			pt_debug(dev, DL_INFO, "%s: Launch APP now...\n",
				__func__);
			ret = cmd->request_pip2_launch_app(dev, 0, 0,
...
static int pt_pip2_upgrade_firmware_from_builtin(struct device *dev)
{
	 ...
	 * 0 - "pt_fw_ram.bin"
	 * 1 - "pt_fw.bin"
	 * 2 - "XXXX_pt_fw<_PIDX>.bin" where XXXX = Silicon ID
	 */
...
#if 1 // (KERNEL_VERSION(3, 13, 0) > LINUX_VERSION_CODE)
		retval = request_firmware(&fw_entry, filename[index], dev);
#else
		retval = request_firmware_direct(&fw_entry,
			filename[index], dev);
...
static DEVICE_ATTR(pip2_bl_from_file, S_IRUGO | S_IWUSR,
...
static DEVICE_ATTR(pip2_file_write, S_IWUSR, NULL, pt_pip2_file_write_store);
#endif /* !TTDL_KERNEL_SUBMISSION */
		...
		char *buf, loff_t offset, size_t count)
{
	...
	if (offset == 0) {
		...
		if (rc) {
			pt_debug(dev, DL_ERROR,
				"%s: Failed to enter BL rc=%d\n",
				__func__, rc);
			goto exit_release;
		}
		pt_debug(dev, DL_INFO,
			"%s Initial mode before entering BL: %d\n",
			__func__, mode);
		...
		ld->pip2_file_data.initial_mode = mode;
...
exit_bl:
	cmd->release_exclusive(dev);
	if (ld->pip2_file_data.initial_mode == PT_MODE_OPERATIONAL) {
		pt_debug(dev, DL_WARN, "%s Requesting RESTART\n", __func__);
		cmd->request_reset(dev, PT_CORE_CMD_UNPROTECTED);
		cmd->request_pip2_launch_app(dev, 0, 0,
			PT_CORE_CMD_UNPROTECTED);
		cmd->request_enum(dev, true);
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	ld->pip2_fcrc.initial_mode = mode;
...
exit_file_close:
	...
	if (file_handle != rc)
		pt_debug(dev, DL_ERROR,
			...
			file_handle, rc);
exit_bl:
	if (ld->pip2_fcrc.initial_mode == PT_MODE_OPERATIONAL) {
		pt_debug(dev, DL_WARN, "%s Requesting Reset\n", __func__);
		cmd->request_reset(dev, PT_CORE_CMD_UNPROTECTED);
		cmd->request_pip2_launch_app(dev, 0, 0,
			PT_CORE_CMD_UNPROTECTED);
		cmd->release_exclusive(dev);
...
static DEVICE_ATTR(pip2_file_crc, S_IRUGO | S_IWUSR,
		...
		struct device_attribute *attr, char *buf)
{
	...
	rc = cmd->request_pip2_enter_bl(dev, &mode);
	if (rc) {
		pt_debug(dev, DL_ERROR, "%s: Failed to enter BL rc=%d\n",
			__func__, rc);
		goto exit_release;
	}
...
exit_bl:
	/*
	 * If FW was not erased and the DUT was in operational mode before the
	 * erase, then queue a restart to get back to operational mode
	 */
	if (file != PIP2_FW_FILE && mode == PT_MODE_OPERATIONAL) {
		pt_debug(dev, DL_WARN, "%s Requesting RESTART\n", __func__);
		cmd->request_reset(dev, PT_CORE_CMD_UNPROTECTED);
		cmd->request_pip2_launch_app(dev, 0, 0,
			PT_CORE_CMD_UNPROTECTED);
...
static DEVICE_ATTR(pip2_file_erase, S_IRUGO | S_IWUSR,
...
#if defined(PT_FW_UPGRADE)
static DEVICE_ATTR(update_fw_status, S_IRUGO, pt_pip2_bl_status_show, NULL);
#endif
/*******************************************************************************
		...
		struct device_attribute *attr, char *buf)
{
	...
	u16 actual_read_len;
	u8 read_buf[256];
	u8 initial_mode = PT_MODE_UNKNOWN;
	struct pip2_cmd_structure pip2_cmd;
