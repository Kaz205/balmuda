...
#include <linux/pt_display.h>
#include <linux/kdisp_tp.h>
...
#define KCTOUCH_STARTUP_TIMEOUT		2000
int kctouch_startup_state = 1;
static int kctouch_ready_flag = 0;

extern struct drm_panel *pt_active_panel;
...
static void pt_si_get_sensing_conf_data(struct pt_core_data *cd)
{
	...
	/* Panel ID coming from XY pin not BL GPIO */
	mutex_lock(&cd->system_lock);
	cd->panel_id = scd_dev->panel_id;
	mutex_unlock(&cd->system_lock);
#endif
...
static int pt_set_touchmode_enabled(struct pt_core_data *cd, u8 mode)
{
	int rc = 0;

	dev_err(cd->dev, "%s: mode=%d\n", __func__, mode);

	if( cd->glove_mode == PT_GLOVE_OFF &&
		mode == PT_TME_FINGER_AND_GLOVE){
		dev_err(cd->dev, "%s: Glove is invalidated\n", __func__);
		return 0;
	}

	if( cd->glove_mode == PT_GLOVE_ON &&
		mode == PT_TME_FINGER_ONLY){
		dev_err(cd->dev, "%s: Glove mode only\n", __func__);
		return 0;
	}

	mutex_lock(&cd->factory_lock);
	mutex_lock(&cd->firmware_class_lock);

    mutex_lock(&cd->system_lock);
    cd->glove_touchmode = mode;
    mutex_unlock(&cd->system_lock);

	if (cd->fb_state == FB_OFF) {
		pr_err("[KCTOUCH]%s: FB OFF -> set skip\n", __func__);
		goto exit;
	}

	if (cd->fw_updating) {
		pr_err("[KCTOUCH]%s: FW loading -> set skip\n", __func__);
		goto exit;
	}

	rc = pt_pip_set_param(cd, PT_RAM_ID_TOUCHMODE_ENABLED, mode, 1);
	if (rc < 0) {
...
exit:
	...
	mutex_unlock(&cd->factory_lock);

	return rc;
}

static int pt_set_gsm_touchmode_enabled(struct pt_core_data *cd, u8 mode)
{
	int rc = 0;
	...
	rc = pt_pip_set_param(cd, PT_RAM_ID_GSM_TOUCHMODE_ENABLED, mode, 1);
	if (rc < 0) {
		dev_err(cd->dev, "%s: failed to set touchmode_enabled rc=%d\n",
				__func__, rc);
	}
...
static int _pt_request_set_touchmode_enabled(struct device *dev, u8 mode)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
...
static void pt_touchmode_work(struct work_struct *work)
{
	struct pt_core_data *cd =
			container_of(work, struct pt_core_data, touchmode_work);
	...
	pt_set_touchmode_enabled(cd, atomic_read(&cd->touchmode_enabled));
}
...
{
#if PT_PANEL_ID_FROM_XY_PIN
	/* The BL is not able to read Panel_ID from an XY pin, use sysinfo */
	if (cd->sysinfo.ready)
		*panel_id = cd->panel_id;
	else
		/* Default to 0 if sysinfo is not ready */
		*panel_id = PT_PANEL_ID_DEFAULT;
	...
	int *actual_wait)
{
	...
	reinit_completion(&cd->exit_bootmode);
...
int _pt_pip2_exit_bl(struct pt_core_data *cd, u8 *status_str, int buf_size)
{
	...
	if (mode == PT_MODE_BOOTLOADER) {
		...
		rc = pt_pip2_launch_app(cd->dev, cd->bl_pip_version_major,
			cd->bl_pip_version_minor, PT_CORE_CMD_UNPROTECTED);
		if (rc) {
			pt_debug(cd->dev, DL_ERROR,
...
reset:
		...
		*enum_status |= STARTUP_STATUS_FW_OUT_OF_BOOT;
		...
		case PT_MODE_BOOTLOADER:
			...
			if (cd->bl_pip_ver_ready == false) {
				...
				pip_major = cd->bl_pip_version_major;
				pip_minor = cd->bl_pip_version_minor;
			}
			 ...
			 * DUT will fail.
			 */
			rc = pt_pip2_launch_app(cd->dev, pip_major, pip_minor,
				PT_CORE_CMD_UNPROTECTED);
			if (rc) {
				pt_debug(cd->dev, DL_ERROR,
					"%s: Error on launch app r=%d\n",
	...
	*enum_status |= STARTUP_STATUS_GET_SYS_INFO;
	...
	mutex_lock(&cd->system_lock);
	set_mode = cd->glove_touchmode;
	mutex_unlock(&cd->system_lock);

	rc = pt_pip_set_param_(cd, PT_RAM_ID_TOUCHMODE_ENABLED, set_mode, 1);
	if (rc < 0) {
		dev_err(cd->dev, "%s: Failed to restore GLOVE touchmode parameter rc=%d\n",
				__func__, rc);
	}else{
		pt_debug(cd->dev, DL_INFO, "%s: Parameter id:%d value:%d\n",
			 __func__, PT_RAM_ID_TOUCHMODE_ENABLED, set_mode);
	}

	mutex_lock(&cd->system_lock);
	set_mode = cd->gsm_touch_mode;
	mutex_unlock(&cd->system_lock);

	rc = pt_pip_set_param_(cd, PT_RAM_ID_GSM_TOUCHMODE_ENABLED, set_mode, 1);
	if (rc < 0) {
		dev_err(cd->dev, "%s: Failed to restore GSM touchmode parameter rc=%d\n",
				__func__, rc);
	}else{
		pt_debug(cd->dev, DL_INFO, "%s: Parameter id:%d value:%d\n",
			 __func__, PT_RAM_ID_GSM_TOUCHMODE_ENABLED, set_mode);
	}
...
exit:
	...
	pr_notice("[KCTOUCH]%s: startup complete set\n", __func__);
	kctouch_startup_state = 1;

	mutex_unlock(&cd->dut_start_lock);
...
static int pt_pinctrl_init(struct pt_core_data *core_data)
{
	int retval = 0;

	core_data->ts_pinctrl = devm_pinctrl_get(core_data->dev);
	if (IS_ERR_OR_NULL(core_data->ts_pinctrl)) {
		pt_debug(core_data->dev, DL_ERROR, "%s: Target does not use pinctrl\n", __func__);
	...
	if (IS_ERR_OR_NULL(core_data->gpio_state_active)) {
		pt_debug(core_data->dev, DL_ERROR, "%s: Can not get ts default pinstate\n", __func__);
	...
	core_data->gpio_state_suspend
		= pinctrl_lookup_state(core_data->ts_pinctrl, "pmx_ts_sleep");
	if (IS_ERR_OR_NULL(core_data->gpio_state_suspend)) {
		pt_debug(core_data->dev, DL_ERROR, "%s: Can not get ts sleep pinstate\n", __func__);
...
int pt_pinctrl_select(struct pt_core_data *core_data, bool on)
{
	struct pinctrl_state *pins_state;
	int ret;

	pr_info("%s is called. bool is %d\n",__func__,on);
	...
	if (!IS_ERR_OR_NULL(pins_state)) {
		...
		if (ret) {
			pt_debug(core_data->dev, DL_ERROR,
				"can not set %s pins\n",
				on ? "pmx_ts_active" : "pmx_ts_sleep");
			return ret;
		}
...
exit:
	if (!cd->flashless_dut && (result == PT_ENTER_BL_PASS)) {
		...
		if (!rc) {
			...
			result = PT_ENTER_BL_GET_FLASH_INFO_FAIL;
			pt_debug(
			    dev, DL_ERROR,
			    "%s: Failed to send PIP2 READ_FLASH_INFO cmd\n",
			    __func__);
		}
	}else if(result == PT_ENTER_BL_CONFIRM_FAIL){
		if (retry_rst-- > 0) {
			mode = PT_MODE_UNKNOWN;
			pt_debug(dev, DL_ERROR, "%s Retry=%d\n", __func__, retry_rst);
			goto _retry;
		}
	}
...
int _pt_request_pip2_bin_hdr(struct device *dev, struct pt_bin_file_hdr *hdr)
{
	...
	if (cd->flashless_dut) {
		...
		rc = _pt_request_pip2_enter_bl(dev, &orig_mode);
		if (rc != PT_ENTER_BL_PASS) {
			ret = PT_BIN_HDR_COMM_ERROR;
			pt_debug(dev, DL_ERROR,
...
exit_close_file:
		...
		if (file_handle != _pt_pip2_file_close(dev, file_handle)) {
			ret = PT_BIN_HDR_FILE_CLOSE_FAIL;
			pt_debug(dev, DL_ERROR,
	...
	hdr->fw_crc      = (read_buf[i+5] << 24) | (read_buf[i+6] << 16) |
			   (read_buf[i+7] << 8)  | (read_buf[i+8]);
...
#ifdef CONFIG_TOUCHSCREEN_KC_TOUCHMODE_ENABLED
	.set_touchmode_enabled         = _pt_request_set_touchmode_enabled,
#endif /* CONFIG_TOUCHSCREEN_KC_TOUCHMODE_ENABLED */
};
 ...
 ******************************************************************************/
static int pt_pip2_read_gpio(struct device *dev, u32 *gpio)
{
	int rc = 0;
	u16 actual_read_len;
	u8 read_buf[12];
	...
	/* Enter BL if not already in the BL */
	pt_debug(dev, DL_INFO, "%s Enter BL\n", __func__);
	...
	pt_pr_buf(dev, DL_WARN, read_buf, ARRAY_SIZE(read_buf), "#2# ");
	rc = _pt_request_pip2_send_cmd(dev,
		PT_CORE_CMD_UNPROTECTED, &pip2_cmd,
		PIP2_CMD_ID_READ_GPIO, NULL, 0, read_buf, &actual_read_len);
	pt_pr_buf(dev, DL_WARN, read_buf, actual_read_len, "#3# ");
...
static int kctp_core_early_suspend(struct device *dev)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	int rc = 0;
	int mode = 0;
	...
	/* Ensure watchdog and startup works stopped */
	cancel_work_sync(&cd->enum_work);
	cancel_work_sync(&cd->kc_resume_work);
	pt_stop_wd_timer(cd);

	if(cd->easywake_valid){
		if(kdisp_touch_get_easywake_onoff(1)){
			mode = 1;
		}else{
			mode = 0;
		}
	...
	{
		/* Easywake */
		pr_info("[KCTOUCH] %s Easywake EarlySuspend\n",__func__);
		cd->enter_lpwg_state = 1;

	}else{
		/* Normal */
		pr_info("[KCTOUCH] %s Normal EarlySuspend\n",__func__);

		cd->enter_lpwg_state = 0;
		...
		cd->irq_disabled = 1;
		udelay(1000);
		pr_info("%s: irq_disabled\n", __func__);
		rc = pt_pinctrl_select(cd, false);
		if(rc){
...
static int kctp_core_suspend(struct device *dev)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	int rc = 0;
	u8 status = 0;

	if (cd->easywake_valid && cd->enter_lpwg_state)
	{
		/* Easywake */
		pr_info("[KCTOUCH] %s Easywake Suspend\n",__func__);

		rc = pt_hid_output_enter_easywake_state_(cd,
			cd->easy_wakeup_gesture, &status);
		if (rc || status == 0){
			pr_err("[KCTOUCH] %s Busy... Retry Enter LPWG\n",__func__);
			rc = pt_hid_output_enter_easywake_state_(cd,
				cd->easy_wakeup_gesture, &status);
		}
		if (rc == 0){
			pr_info("[KCTOUCH] %s Enter LPWG\n",__func__);
		}
	}else{
		/* Normal */
		pr_info("[KCTOUCH] %s Normal Suspend\n",__func__);
	}

	return rc;
}

static int kctp_core_early_resume(struct device *dev)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	int rc = 0;
	int mode = 0;
	...
	if(cd->easywake_valid){
		if(kdisp_touch_get_easywake_onoff(0)){
			mode = 1;
		}else{
			mode = 0;
		}
	...
	{
		/* Easywake */
		pr_info("[KCTOUCH] %s Easywake EarlyResume\n",__func__);
		cd->enter_lpwg_state = 1;

		rc = pt_put_device_into_deep_sleep_(cd);
		pr_info("[KCTOUCH] %s Core Wakeup(Deep Sleep)\n",__func__);
		mdelay(5);

		rc = pt_core_wake_device_from_deep_sleep_(cd);
		pr_info("[KCTOUCH] %s Core Wakeup(Exit LPWG)\n",__func__);
		mdelay(20);
	}else{
		/* Normal */
		pr_info("[KCTOUCH] %s Normal EarlyResume\n",__func__);
...
static int kctp_core_resume(struct device *dev)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	int rc = 0;
    ...
    mutex_lock(&cd->dut_start_lock);
		...
		if (cd->fb_state != FB_ON) {
			if(cd->glove_mode == PT_GLOVE_OFF){
				...
				call_atten_cb(cd, PT_ATTEN_FINGER, 0);
			} else {
				dev_info(cd->dev, "%s: Force GLOVE ON\n", __func__);
				call_atten_cb(cd, PT_ATTEN_GLOVE, 0);
			}
...
void pt_display_set_easywake_mode(void *pt_data, int easywake_mode){
	struct pt_core_data *cd;

	pr_info("[KCTOUCH] %s easywake_mode=%d\n",__func__, easywake_mode);

	cd = (struct pt_core_data*)pt_data;
	
	cd->easy_wakeup_gesture = easywake_mode;
	
}

int pt_display_resume(void *pt_data, int order){
	struct pt_core_data *cd;
	int rc = 0;
	...
	case 2:
		break;
	case 3:
		rc = pt_pinctrl_select(cd, true);
		udelay(500);
		if(rc){
			pr_err("%s pinctrl Fail.\n", __func__);
		}

		if(cd->irq_disabled  == 1)
		{				
			enable_irq(cd->irq);
			cd->irq_disabled = 0;
		}else{
			pr_info("%s: irq_enabled skip\n", __func__);
		}
		break;
	default:
		pr_err("%s touch order error(%d)\n",__func__, order);
		break;
	}
...
int pt_display_suspend(void *pt_data, int order){
	struct pt_core_data *cd;
...
static void kc_resume_work_function(struct work_struct *work)
{
	struct pt_core_data *cd =  container_of(work,
		struct pt_core_data, kc_resume_work);
	int rc;

	rc = kctp_core_resume(cd->dev);
	if (rc < 0)
		pt_debug(cd->dev, DL_ERROR, "%s: Fail queued resume_work rc=%d\n",
			__func__, rc);
}
		...
		unsigned long event, void *data)
{
	...
	if(cd->factory_state){
		pr_info("%s: Factory_State Locked!\n", __func__);
		goto factory_lock_exit;
	}

	if (!kctouch_startup_state) {
		for (i=0;i<KCTOUCH_STARTUP_TIMEOUT;i++) {
			if (kctouch_startup_state) {
				pr_notice("[KCTOUCH]%s: startup complete count=%d\n", __func__, i);
				break;
			}
			msleep(10);
		}

		if(i>=KCTOUCH_STARTUP_TIMEOUT) {
			pr_notice("[KCTOUCH]%s: startup complete timeout\n", __func__);
		}
	}

	blank = evdata->data;
	
	pr_err("FB event:%lu,blank:%d", event, *blank);
	switch (*blank) {
		case DRM_PANEL_BLANK_UNBLANK:
			pt_debug(cd->dev, DL_ERROR, "%s: UNBLANK!\n", __func__);

			if (event == DRM_PANEL_EARLY_EVENT_BLANK) {
//				pr_err("resume: event = %lu, DRM_PANEL_EARLY_EVENT_BLANK\n", event);
				kctp_core_early_resume(cd->dev);
			} else if (event == DRM_PANEL_EVENT_BLANK) {
//				pr_err("resume: event = %lu, DRM_PANEL_EVENT_BLANK\n", event);
				schedule_work(&cd->kc_resume_work);
			}

			break;
			
		case DRM_PANEL_BLANK_POWERDOWN:
			pt_debug(cd->dev, DL_ERROR, "%s: POWERDOWN!\n", __func__);

            mutex_lock(&cd->dut_start_lock);

			if (event == DRM_PANEL_EARLY_EVENT_BLANK) {
//				pr_err("suspend: event = %lu, DRM_PANEL_EARLY_EVENT_BLANK\n", event);
				kctp_core_early_suspend(cd->dev);
			} else if (event == DRM_PANEL_EVENT_BLANK) {
//				pr_err("suspend: event = %lu, DRM_PANEL_EVENT_BLANK\n", event);
				kctp_core_suspend(cd->dev);
				if (!kctouch_ready_flag) {
					kctouch_ready_flag = 1;
				}
			}
		...
		default:
			pr_err("FB BLANK(%d) do not need process\n", *blank);
			break;
	}
...
static void pt_setup_fb_notifier(struct pt_core_data *cd)
{
	...
	if (pt_active_panel) {
		...
		rc = drm_panel_notifier_register(pt_active_panel, &cd->fb_notifier);
		if (rc < 0) {
			pt_debug(cd->dev, DL_ERROR,
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if(cd->gsm_touch_mode != mode)
		pt_set_gsm_touchmode_enabled(cd, mode);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	case KC_TOUCH_FACTORY_LOCK:
		...
		dev_info(dev, "%s: enum_work +\n", __func__);
		if (flush_work(&cd->enum_work))
			pr_info("%s: waited enum_work finished\n", __func__);
		dev_info(dev, "%s: enum_work -\n", __func__);

		dev_info(dev, "%s: kc_resume_work +\n", __func__);
		if (flush_work(&cd->kc_resume_work))
			pr_info("%s: waited kc_resume_work finished\n", __func__);
		dev_info(dev, "%s: kc_resume_work -\n", __func__);

		status = wait_for_completion_interruptible_timeout(&cd->exit_bootmode, 20 * HZ);
		if(status<=0){
			pt_debug(dev, DL_ERROR, "%s: Error Exit Bootmode timeout. rc = %d\n", __func__, rc);
			rc = -EINVAL;
			break;
		}
		
		dev_info(dev, "%s: WDT force stop\n", __func__);
		wd_disabled = 1;
		cd->watchdog_force_stop = true;
	...
	case KC_TOUCH_POWER_ON:
		dev_info(dev, "%s: KC_TOUCH_POWER_ON (cd=%p)\n", __func__, cd);
		break;
	case KC_TOUCH_POWER_OFF:
		dev_info(dev, "%s: KC_TOUCH_POWER_OFF (cd=%p)\n", __func__, cd);
		break;
	default:
		rc = -EINVAL;
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (cd->active_dut_generation == DUT_PIP1_ONLY) {
		if (cd->mode == PT_MODE_BOOTLOADER) {
			rc = pt_hid_output_bl_get_panel_id_(cd, &cd->panel_id);
			if (rc) {
		...
		if (cd->mode == PT_MODE_BOOTLOADER) {
			rc = pt_pip2_read_gpio(dev, &gpio_value);
			pid = (gpio_value & PT_PANEL_ID_BITMASK) >>
				PT_PANEL_ID_SHIFT;
			pt_debug(dev, DL_WARN,
				"%s: BL mode PID=0x%02X gpio=0x%08X\n",
				__func__, pid, gpio_value);
		} else if (cd->mode == PT_MODE_OPERATIONAL) {
			rc = pt_hid_output_get_sysinfo(cd);
			if (!rc)
				pid = cd->panel_id;
...
static ssize_t pt_under_water_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	struct pt_mt_data *md = &cd->md;
...
#endif // CONFIG_TOUCHSCREEN_UNDERWATER

#ifdef CONFIG_TOUCHSCREEN_KC_TOUCHMODE_IOCTL
static ssize_t pt_touchmode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	ssize_t ret;
	pt_debug(cd->dev, DL_DEBUG, "%s is called\n",__func__);

	if(atomic_read(&cd->force_finger))
		ret = sprintf(buf, "Force Finger ON\n");
	else
		ret = sprintf(buf, "Force Finger OFF\n");

	return ret;
}

static ssize_t pt_touchmode_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	unsigned long value;
	...
	if (retval < 0 || value > 1) {
		pt_debug(dev, DL_ERROR, "%s: Invalid value\n", __func__);
		goto pt_touchmode_store_exit;
	}

	/*******************/
	atomic_set(&cd->oem_state, 1);
	/*******************/

	if (value) {
		pt_debug(cd->dev, DL_INFO, "%s: Force Finger ON\n", __func__);
		atomic_set(&cd->force_finger, 1);
		if (atomic_read(&cd->oem_state))
			call_atten_cb(cd, PT_ATTEN_FINGER, 0);
	}
	else {
		pt_debug(cd->dev, DL_INFO, "%s: Force Finger OFF\n", __func__);
		atomic_set(&cd->force_finger, 0);
		if (atomic_read(&cd->oem_state))
			call_atten_cb(cd, PT_ATTEN_GLOVE, 0);
	}

pt_touchmode_store_exit:
	return size;
}
#endif // CONFIG_TOUCHSCREEN_KC_TOUCHMODE_IOCTL

static ssize_t pt_glove_mode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	ssize_t ret;

	ret = snprintf(buf, PT_MAX_PRBUF_SIZE, "%x\n", cd->glove_mode);
	pr_info("%s: glove_mode = %d\n",__func__, cd->glove_mode);
	return ret;
}

static ssize_t pt_glove_mode_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	unsigned int buf_tmp;
	struct pt_core_data *cd = dev_get_drvdata(dev);

	sscanf(buf, "%x", &buf_tmp);

	if ( buf_tmp != PT_GLOVE_OFF &&
		 buf_tmp != PT_GLOVE_ON  &&
		 buf_tmp != PT_GLOVE_AUTO){
		pr_err("%s: Error: invalid value\n",__func__);
		return -EINVAL;
	} else {
		sscanf(buf, "%x", (unsigned int *) &cd->glove_mode);
	}
	if(cd->glove_mode == PT_GLOVE_OFF){
		dev_info(cd->dev, "%s: Force Finger ON\n", __func__);
		call_atten_cb(cd, PT_ATTEN_FINGER, 0);
...
static ssize_t kc_xres_test_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	ssize_t ret;
	...
	/* Count the number of tests to run */
//	tests = cd->ttdl_bist_select;
	tests = PT_TTDL_BIST_TP_XRES_TEST;
	while (tests) {
		num_tests += tests & 1;
		tests >>= 1;
	}
...
#if 1
		ret = sprintf(buf, "%d\n", 
			xres_toggled == 0x0F ? -ENOMEM :
				xres_toggled == 1 ? 0 : 1);

		pt_debug(dev, DL_WARN, "%s: status = %d (%d) err = %s\n",
			__func__, status, xres_toggled, xres_err_str);
 ...
 *	 size - size of buf
 ******************************************************************************/
static ssize_t kc_xres_test_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	u32 input_data[2] = {0};
...
static DEVICE_ATTR(xres_test,  0644, kc_xres_test_show,
	kc_xres_test_store);
...
static ssize_t pt_kc_ready_status_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t ret;

	ret = snprintf(buf, PT_MAX_PRBUF_SIZE, "%x\n", kctouch_ready_flag);
	pr_info("%s: kctouch_ready_flag = %d\n",__func__, kctouch_ready_flag);
	return ret;
}
...
#endif
#ifdef CONFIG_TOUCHSCREEN_UNDERWATER
	__ATTR(under_water, S_IRUSR|S_IRGRP, pt_under_water_show, NULL),
#endif
#ifdef CONFIG_TOUCHSCREEN_KC_TOUCHMODE_IOCTL
	__ATTR(touch_mode, S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP,
			pt_touchmode_show, pt_touchmode_store),
#endif
	__ATTR(glove_mode, S_IRUSR | S_IRGRP | S_IWUSR | S_IWGRP,
		 pt_glove_mode_show, pt_glove_mode_store),
#ifdef TTDL_DIAGNOSTICS
...
#endif /* TTDL_DIAGNOSTICS */
	__ATTR(kc_ready_status, 0444, pt_kc_ready_status_show, NULL),
};
...
static void pt_get_touch_info(struct kc_ts_data *ts)
{
	return;
}

static int pt_mode_check_ioctl(struct kc_ts_data *ts)
{
#if 1
	struct pt_core_data *cd = (struct pt_core_data *)ts->vdata;
	int rc;
	...
	rc = cd->mode;
	if (rc == PT_MODE_OPERATIONAL)
		dev_dbg(cd->dev, "%s: Operational mode\n", __func__);
	else if (rc == PT_MODE_BOOTLOADER)
		dev_info(cd->dev, "%s: Bootloader mode\n", __func__);
	else if (rc == PT_MODE_UNKNOWN) {
...
static int pt_get_system_info_ioctl(struct kc_ts_data *ts, u8 *dp)
{
	return 0;
}

#ifdef CONFIG_TOUCHSCREEN_KC_SENSOR_SWITCH
static int pt_sensor_switch_ioctl(struct kc_ts_data *ts, int value)
{
	return 0;
}
#endif /* CONFIG_TOUCHSCREEN_KC_SENSOR_SWITCH */
...
#ifdef CONFIG_TOUCHSCREEN_KC_TOUCHMODE_IOCTL
static int pt_set_touchmode_ioctl(struct kc_ts_data *ts, int value)
{
	return 0;
}
#endif /* CONFIG_TOUCHSCREEN_KC_TOUCHMODE_IOCTL */
...
static const struct kc_ts_operations pt_operations = {
	.get_touch_info		= pt_get_touch_info,
	.mode_check			= pt_mode_check_ioctl,
	.get_system_info	= pt_get_system_info_ioctl,
#ifdef CONFIG_TOUCHSCREEN_KC_SENSOR_SWITCH
	.sensor_switch	= pt_sensor_switch_ioctl,
#endif /* CONFIG_TOUCHSCREEN_KC_SENSOR_SWITCH */
#ifdef CONFIG_TOUCHSCREEN_KC_TOUCHMODE_IOCTL
	.set_touchmode	= pt_set_touchmode_ioctl,
#endif /* CONFIG_TOUCHSCREEN_KC_TOUCHMODE_IOCTL */
};
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	mutex_init(&cd->factory_lock);
	mutex_init(&cd->dut_start_lock);
	mutex_init(&cd->kcsysfs_lock);
	init_completion(&cd->exit_bootmode);
	spin_lock_init(&cd->spinlock);

	cd->factory_state = false;
	cd->ts_event_control = false;
	...
	INIT_WORK(&cd->kc_resume_work, kc_resume_work_function);
#ifdef CONFIG_TOUCHSCREEN_KC_TOUCHMODE_ENABLED
	cd->glove_valid = cd->cpdata->glove_valid;
	cd->glove_mode = PT_GLOVE_OFF;
	cd->glove_touchmode = PT_TME_FINGER_ONLY;
	cd->gsm_touch_mode = 0;
	INIT_WORK(&cd->touchmode_work, pt_touchmode_work);
	atomic_set(&cd->touchmode_enabled, PT_TME_NULL);
#endif /* CONFIG_TOUCHSCREEN_KC_TOUCHMODE_ENABLED */
	...
	cd->enter_lpwg_state = 0;
	cd->easywake_valid = cd->cpdata->easywake_valid;

#if PT_PANEL_ID_FROM_XY_PIN
	/* Set Panel ID to default to 0 */
	cd->panel_id = PT_PANEL_ID_DEFAULT;
#else
