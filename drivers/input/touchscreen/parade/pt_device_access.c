...
start_testing:
	...
	if (!dad->cmcp_force_calibrate) {
		...
		if (rc < 0) {
			pt_debug(dev, DL_ERROR,
				"%s: Error calibrating idacs for mutual rc=%d\n",
				__func__, rc);
		}
		rc = _pt_calibrate_idacs_cmd(dev, 1, &status);
		if (rc < 0) {
			pt_debug(dev, DL_ERROR,
				"%s: Error calibrating idacs for btns rc=%d\n",
				__func__, rc);
		}
		rc = _pt_calibrate_idacs_cmd(dev, 2, &status);
		if (rc < 0) {
			pt_debug(dev, DL_ERROR,
				"%s: Error calibrating idacs for self rc=%d\n",
				__func__, rc);
		}
	}
	/*resume_scanning */
	...
	self_test_id_supported =
		pt_get_cmcp_info(dad, cmcp_info);
	...
	if (result->test_summary) {
		pt_debug(dev, DL_INFO, "%s: Finish Cm/Cp test! All Test Passed\n",
				__func__);
		index = snprintf(buf, PT_MAX_PRBUF_SIZE,
				"Status: 1\n");
	} else {
		pt_debug(dev, DL_INFO, "%s: Finish Cm/Cp test! Range Check Failure\n",
				__func__);
		index = snprintf(buf, PT_MAX_PRBUF_SIZE,
				"Status: 6\n");
	}
...
static DEVICE_ATTR(cmcp_threshold_loading, S_IWUSR,
	NULL, pt_cmcp_threshold_loading_store);


static ssize_t kc_manufacturing_test_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_device_access_data *dad
		= pt_get_device_access_data(dev);
	...
	pt_debug(dev, DL_ERROR, "%s: START\n", __func__);

	mutex_lock(&dad->sysfs_lock);

	pr_err("[KCTEST]%s: size=%d\n",__func__ , size);
	mutex_unlock(&dad->sysfs_lock);

	pt_debug(dev, DL_ERROR, "%s: END\n", __func__);

	return size;
}

static ssize_t kc_manufacturing_test_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct pt_device_access_data *dad
		= pt_get_device_access_data(dev);
...
static DEVICE_ATTR(manufacturing_test, S_IRUSR | S_IWUSR,
	kc_manufacturing_test_show, kc_manufacturing_test_store);
		...
		u8 *out_buf, size_t out_buf_size, u8 out_format)
{
	...
	for (i=0; i<10; i++) {
		pr_err("[KCTEST]%s: in_buf[%d]=%02X %02X %02X %02X %02X\n",__func__ ,
				j, in_buf[j], in_buf[j+1], in_buf[j+2], in_buf[j+3], in_buf[j+4]);
		j+=5;
	}
	...
	if (out_format == PT_PR_FORMAT_DEFAULT) {
		...
		j=0;
		for (i=0; i<10; i++) {
			pr_err("[KCTEST]%s: out_buf[%d]=%02X %02X %02X %02X %02X\n",__func__ ,
					j, out_buf[j], out_buf[j+1], out_buf[j+2], out_buf[j+3], out_buf[j+4]);
			j+=5;
		}
	...
	} else if (out_format == PT_PR_FORMAT_DEFAULT_1) {
		for (i = 0; i < length; i++) {
			out_buf[i] = (char)in_buf[i];
			index = length;
		}
		...
		j=0;
		for (i=0; i<10; i++) {
			pr_err("[KCTEST]%s:#2 out_buf[%d]=%02X %02X %02X %02X %02X\n",__func__ ,
					j, out_buf[j], out_buf[j+1], out_buf[j+2], out_buf[j+3], out_buf[j+4]);
			j+=5;
		}
	}
...
put_pm_runtime:
	...
	for (i = 0; i < length; i++)
	{
		buf_out[i] = (char)dad->ic_buf[i];
	}
	print_idx = length;
		pt_debug(dev, DL_ERROR, "%s: Sent %d bytes to user space\n",
			__func__, print_idx);
#endif
...
exit:
	pr_err("[KCTEST]%s: length=%d\n",__func__ , data->pr_buf_len);

	j=0;
	for(i=0; i<(data->pr_buf_len/5); i++){
		pr_err("[KCTEST][x]%s: ic_buf[%d] %02X %02X %02X %02X %02X\n",
				__func__ , j, data->pr_buf[j], data->pr_buf[j+1], data->pr_buf[j+2], data->pr_buf[j+3], data->pr_buf[j+4]);
	}
...
#if 1 // KC_CUSTOM
static ssize_t fw_self_test_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	u8 ret_status;
	u8 ret_self_test_id;
	...
	data = kzalloc(sizeof(*data), GFP_KERNEL);
	if (!data) {
		pt_debug(dad->dev, DL_ERROR, "%s: error alloc data\n", __func__);
		goto end;
	}

	mutex_lock(&cd->kcsysfs_lock);
	data->dad = dad;
...
exit:
	length = data->pr_buf_len;
	memcpy(buf, data->pr_buf, data->pr_buf_len);
	mutex_unlock(&cd->kcsysfs_lock);
	kfree(data);

end:
	pr_err("[KCTOUCH]%s: size=%d rc=%d\n",__func__ , length, rc);
	return length;
}

static ssize_t fw_self_test_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct pt_device_access_debugfs_data *data;
	struct pt_device_access_data *dad = pt_get_device_access_data(dev);
	...
	mutex_lock(&cd->kcsysfs_lock);
	data->dad = dad;
	memcpy(data->pr_buf, buf, size);

	length = cmd->parse_sysfs_input(dad->dev, data->pr_buf, size,
		input_data, PT_FW_SELF_TEST_MAX_PARM + 1);
	if (length == 1) {
		dad->fw_self_test_id = input_data[0];
		dad->fw_self_test_format = PT_PR_FORMAT_DEFAULT;
...
static ssize_t panel_scan_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_device_access_data *dad = pt_get_device_access_data(dev);
	struct pt_core_data *cd = dev_get_drvdata(dev);
...
put_pm_runtime:
	 ...
	 */
	dad->panel_scan_size = print_idx;
	memcpy(buf, buf_out, print_idx);

release_mutex:
	mutex_unlock(&cd->kcsysfs_lock);
	pr_err("[KCTOUCH]%s: size=%d rc=%d\n",__func__ , print_idx, rc);
	return print_idx;
}

static ssize_t panel_scan_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct pt_device_access_debugfs_data *data;
	struct pt_device_access_data *dad = pt_get_device_access_data(dev);
...
static ssize_t get_idac_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_device_access_debugfs_data *data;
	struct pt_device_access_data *dad = pt_get_device_access_data(dev);
	...
	pr_err("[KCTEST]%s: get_idac_data_id=%d\n",__func__ , dad->get_idac_data_id);
...
static ssize_t get_idac_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct pt_device_access_debugfs_data *data;
	struct pt_device_access_data *dad = pt_get_device_access_data(dev);
...
static ssize_t opens_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_device_access_debugfs_data *data;
	struct pt_device_access_data *dad = pt_get_device_access_data(dev);
...
static ssize_t calibrate_ext_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_device_access_debugfs_data *data;
	struct pt_device_access_data *dad = pt_get_device_access_data(dev);
...
static ssize_t calibrate_ext_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct pt_device_access_debugfs_data *data;
	struct pt_device_access_data *dad = pt_get_device_access_data(dev);
...
static ssize_t baseline_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct pt_device_access_debugfs_data *data;
	struct pt_device_access_data *dad = pt_get_device_access_data(dev);
...
static ssize_t cm_button_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_device_access_debugfs_data *data;
	struct pt_device_access_data *dad = pt_get_device_access_data(dev);
...
static ssize_t cp_button_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_device_access_debugfs_data *data;
	struct pt_device_access_data *dad = pt_get_device_access_data(dev);
...
static ssize_t cmcp_results_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t length = 0;

//	pt_debug(dad->dev, DL_ERROR, "%s: Not Supported\n", __func__);

	return length;
}

static DEVICE_ATTR(kc_fw_self_test, S_IRUSR |S_IRGRP |S_IROTH | S_IWUSR, fw_self_test_show, fw_self_test_store);
static DEVICE_ATTR(kc_panel_scan, S_IRUSR |S_IRGRP |S_IROTH | S_IWUSR, panel_scan_show, panel_scan_store);
static DEVICE_ATTR(kc_get_idac, S_IRUSR |S_IRGRP |S_IROTH | S_IWUSR, get_idac_show, get_idac_store);
static DEVICE_ATTR(kc_auto_shorts, S_IRUSR |S_IRGRP |S_IROTH , auto_shorts_show, NULL);
static DEVICE_ATTR(kc_opens, S_IRUSR |S_IRGRP |S_IROTH , opens_show, NULL);
static DEVICE_ATTR(kc_calibrate_ext, S_IRUSR |S_IRGRP |S_IROTH | S_IWUSR, calibrate_ext_show, calibrate_ext_store);
static DEVICE_ATTR(kc_calibrate, S_IRUSR |S_IRGRP |S_IROTH | S_IWUSR, calibrate_show, calibrate_store);
static DEVICE_ATTR(kc_baseline, S_IRUSR |S_IRGRP |S_IROTH | S_IWUSR, baseline_show, baseline_store);
static DEVICE_ATTR(kc_cm_panel, S_IRUSR |S_IRGRP |S_IROTH , cm_panel_show, NULL);
static DEVICE_ATTR(kc_cp_panel, S_IRUSR |S_IRGRP |S_IROTH , cp_panel_show, NULL);
static DEVICE_ATTR(kc_cm_button, S_IRUSR |S_IRGRP |S_IROTH , cm_button_show, NULL);
static DEVICE_ATTR(kc_cp_button, S_IRUSR |S_IRGRP |S_IROTH , cp_button_show, NULL);
static DEVICE_ATTR(kc_cmcp_results, S_IRUSR |S_IRGRP |S_IROTH , cmcp_results_show, NULL);
#endif // KC_CUSTOM
...
static int pt_setup_sysfs(struct device *dev)
{
	...
	if (rc) {
		pt_debug(dev, DL_ERROR,
			"%s: Error, could not create manufacturing_test\n",
			__func__);
		goto unregister_cmcp_thresold_loading;
	}

	rc = device_create_file(dev, &dev_attr_kc_fw_self_test);
	if (rc) {
		pt_debug(dev, DL_ERROR,
			"%s: Error, could not create kc_fw_self_test\n",
			__func__);
		goto unregister_manufacturing_test;
	}

	rc = device_create_file(dev, &dev_attr_kc_panel_scan);
	if (rc) {
		pt_debug(dev, DL_ERROR,
			"%s: Error, could not create kc_panel_scan\n",
			__func__);
		goto unregister_kc_fw_self_test;
	}

	rc = device_create_file(dev, &dev_attr_kc_get_idac);
	if (rc) {
		pt_debug(dev, DL_ERROR,
			"%s: Error, could not create kc_get_idac\n",
			__func__);
		goto unregistre_kc_panel_scan;
	}

	rc = device_create_file(dev, &dev_attr_kc_auto_shorts);
	if (rc) {
		pt_debug(dev, DL_ERROR,
			"%s: Error, could not create kc_auto_shorts\n",
			__func__);
		goto unregistre_kc_get_idac;
	}

	rc = device_create_file(dev, &dev_attr_kc_opens);
	if (rc) {
		pt_debug(dev, DL_ERROR,
			"%s: Error, could not create kc_opens\n",
			__func__);
		goto unregistre_kc_auto_shorts;
	}

	rc = device_create_file(dev, &dev_attr_kc_calibrate_ext);
	if (rc) {
		pt_debug(dev, DL_ERROR,
			"%s: Error, could not create kc_calibrate_ext\n",
			__func__);
		goto unregistre_kc_opens;
	}

	rc = device_create_file(dev, &dev_attr_kc_calibrate);
	if (rc) {
		pt_debug(dev, DL_ERROR,
			"%s: Error, could not create kc_calibrate\n",
			__func__);
		goto unregistre_kc_calibrate_ext;
	}

	rc = device_create_file(dev, &dev_attr_kc_baseline);
	if (rc) {
		pt_debug(dev, DL_ERROR,
			"%s: Error, could not create kc_baseline\n",
			__func__);
		goto unregistre_kc_calibrate;
	}

	rc = device_create_file(dev, &dev_attr_kc_cm_panel);
	if (rc) {
		pt_debug(dev, DL_ERROR,
			"%s: Error, could not create kc_cm_panel\n",
			__func__);
		goto unregistre_kc_baseline;
	}

	rc = device_create_file(dev, &dev_attr_kc_cp_panel);
	if (rc) {
		pt_debug(dev, DL_ERROR,
			"%s: Error, could not create kc_cp_panel\n",
			__func__);
		goto unregistre_kc_cm_panel;
	}

	rc = device_create_file(dev, &dev_attr_kc_cm_button);
	if (rc) {
		pt_debug(dev, DL_ERROR,
			"%s: Error, could not create kc_cm_button\n",
			__func__);
		goto unregistre_kc_cp_panel;
	}

	rc = device_create_file(dev, &dev_attr_kc_cp_button);
	if (rc) {
		pt_debug(dev, DL_ERROR,
			"%s: Error, could not create kc_cp_button\n",
			__func__);
		goto unregistre_kc_cm_button;
	}

	rc = device_create_file(dev, &dev_attr_kc_cmcp_results);
	if (rc) {
		pt_debug(dev, DL_ERROR,
			"%s: Error, could not create kc_cmcp_results\n",
			__func__);
		goto unregistre_kc_cp_button;
	}

	dad->sysfs_nodes_created = true;
	return rc;

unregistre_kc_cp_button:
	device_remove_file(dev, &dev_attr_kc_cp_button);
unregistre_kc_cm_button:
	device_remove_file(dev, &dev_attr_kc_cm_button);
unregistre_kc_cp_panel:
	device_remove_file(dev, &dev_attr_kc_cp_panel);
unregistre_kc_cm_panel:
	device_remove_file(dev, &dev_attr_kc_cm_panel);
unregistre_kc_baseline:
	device_remove_file(dev, &dev_attr_kc_baseline);
unregistre_kc_calibrate:
	device_remove_file(dev, &dev_attr_kc_calibrate);
unregistre_kc_calibrate_ext:
	device_remove_file(dev, &dev_attr_kc_calibrate_ext);
unregistre_kc_opens:
	device_remove_file(dev, &dev_attr_kc_opens);
unregistre_kc_auto_shorts:
	device_remove_file(dev, &dev_attr_kc_auto_shorts);
unregistre_kc_get_idac:
	device_remove_file(dev, &dev_attr_kc_get_idac);
unregistre_kc_panel_scan:
	device_remove_file(dev, &dev_attr_kc_panel_scan);
unregister_kc_fw_self_test:
	device_remove_file(dev, &dev_attr_kc_fw_self_test);
...
//	mutex_init(&cd->kcsysfs_lock);
	...
	*data = dad;
	...
	if (dad->si) {
		...
		rc = pt_setup_sysfs(dev);
		if (rc)
			pt_debug(dev, DL_ERROR, "[KCTOUCH]%s pt_setup_sysfs error\n", __func__);
