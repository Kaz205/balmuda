...
#ifdef KCTP_CUST
		fw_img_ver = tfd.new_fw_cb;
		ilits->kctp_data.fw_img_ver = fw_img_ver;
		ILI_INFO("FW IMG ver = 0x%x, FW IC ver = 0x%x\n", ilits->kctp_data.fw_img_ver, ilits->chip->fw_ver);
#endif /* KCTP_CUST */
...
static int ilitek_fw_flash_upgrade(u8 *pfw, bool mcu)
{
	...
	if (ret == UPDATE_PASS) {
		ILI_INFO("Flash FW and target file FW are same, SKIP upgrade\n");
		if (ili_ice_mode_ctrl(DISABLE, mcu) < 0) {
