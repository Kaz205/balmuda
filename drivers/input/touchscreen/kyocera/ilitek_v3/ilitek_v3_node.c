...
#ifdef KCTP_CUST
	len += snprintf(g_user_buf + len, USER_STR_BUFF - len, "(%d)\n", ret);
#endif /* KCTP_CUST */
...
#ifdef KCTP_CUST
#define KCTOUCH_MP_OFF_TEST_TIMEOUT 1000
#endif /* KCTP_CUST */
...
#ifdef KCTP_CUST
	ILI_INFO("Enter MP test with LCM off (gesture is %d)\n", ilits->gesture);

	if (ilits->tp_suspend == false) {
		ILI_INFO("waiting... tp_suspend=%d\n", ilits->tp_suspend);

		for (i=0; i<KCTOUCH_MP_OFF_TEST_TIMEOUT; i++) {
			if (ilits->tp_suspend == true) {
				ILI_INFO("tp_suspend end. count=%d\n", i);
				break;
			}
			msleep(10);
		}
	}else{
		ILI_INFO("tp_suspend end\n");
	}
...
static ssize_t ilitek_proc_sram_test_info(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	if (((ilits->chip->pid & 0xFFFFFF00) == ILI9881N_AA)
			|| ((ilits->chip->pid & 0xFFFFFF00) == ILI7807Q_AA)) {
		/* 7807Q 9881N do nothing */
	} else {
		ILI_INFO("close FW SRAM power saving\n");
		ili_ice_mode_write(0x8003F, 0x8A, 1);
		ili_ice_mode_write(0x8003C, 0x00, 1);
	}
