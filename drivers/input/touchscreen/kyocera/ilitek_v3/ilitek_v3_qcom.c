...
#ifdef KCTP_CUST
	input_set_capability(ilits->input, EV_KEY, KEY_F3);
	__set_bit(KEY_F3, ilits->input->keybit);
#endif /* KCTP_CUST */
...
#ifdef KCTP_CUST
	ret = of_property_read_u32(dev_node, "kc,func_mode", &flag);
	if(!ret){
		ILI_INFO("TP MODE: Valid data = %d\n", flag);
		ilits->kc_func_mode = flag;
	}else{
		ILI_INFO("TP MODE: INVALID!\n");
		ilits->kc_func_mode = 0;
	}
	ILI_INFO("TP MODE: %d\n", ilits->kc_func_mode);
#endif /* KCTP_CUST */
...
#ifdef KCTP_CUST
#define KCTOUCH_FW_STAT_TIMEOUT		1000
static ssize_t kctp_fw_version_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	u32 length = 0;
	...
	ILI_INFO("kctp_fw_version_show\n");

	if (atomic_read(&ilits->fw_stat)) {
		ILI_INFO("Not ready...\n");
		for (i=0;i<KCTOUCH_FW_STAT_TIMEOUT;i++) {
			if (atomic_read(&ilits->fw_stat) == END) {
				ILI_INFO("FW version check ready! count=%d\n", i);
				break;
			}
			msleep(10);
		}
	}else{
		ILI_INFO("FW version check ready!\n");
	}

	fw_ic_ver = ilits->chip->fw_ver;
	fw_img_ver = KCTOUCH_FW_BIN_VERSION;

	ILI_INFO("fw_ic_ver: 0x%x\n", fw_ic_ver);
	ILI_INFO("fw_img_ver: 0x%x\n", fw_img_ver);

	if(fw_img_ver != fw_ic_ver) {
		ILI_ERR("FW version is not same!\n");
		status = 1;
	}else{
		ILI_INFO("FW version is same.\n");
		status = STATUS_SUCCESS;
	}

	length += snprintf(buf+length, KTP_MAX_PRBUF_SIZE-length, "Result: %s\n", (status==STATUS_SUCCESS) ? KTP_RET_OK : KTP_RET_NG);

	length += snprintf(buf+length, KTP_MAX_PRBUF_SIZE-length, "CHIP ID = %x\n", ilits->chip->id);
	length += snprintf(buf+length, KTP_MAX_PRBUF_SIZE-length, "FW version = %d.%d.%d.%d\n",
			ilits->chip->fw_ver >> 24, (ilits->chip->fw_ver >> 16) & 0xFF,
			(ilits->chip->fw_ver >> 8) & 0xFF, ilits->chip->fw_ver & 0xFF);

	length += snprintf(buf+length, KTP_MAX_PRBUF_SIZE-length, "FW IMG version = %d.%d.%d.%d\n",
			ilits->kctp_data.fw_img_ver >> 24, (ilits->kctp_data.fw_img_ver >> 16) & 0xFF,
			(ilits->kctp_data.fw_img_ver >> 8) & 0xFF, ilits->kctp_data.fw_img_ver & 0xFF);

	length += snprintf(buf+length, KTP_MAX_PRBUF_SIZE-length, "FW MP version = %d.%d.%d.%d\n",
	...
	if (ilits->protocol->core_ver_len == P5_X_CORE_VER_THREE_LENGTH)
		length += snprintf(buf+length, KTP_MAX_PRBUF_SIZE-length, "Core version = %d.%d.%d\n",
			(ilits->chip->core_ver >> 24) & 0xFF, (ilits->chip->core_ver >> 16) & 0xFF,
			(ilits->chip->core_ver >> 8) & 0xFF);
	else
		length += snprintf(buf+length, KTP_MAX_PRBUF_SIZE-length, "Core version = %d.%d.%d.%d\n",
			(ilits->chip->core_ver >> 24) & 0xFF, (ilits->chip->core_ver >> 16) & 0xFF,
			(ilits->chip->core_ver >> 8) & 0xFF, ilits->chip->core_ver & 0xFF);

	length += snprintf(buf+length, KTP_MAX_PRBUF_SIZE-length, "Protocol version = %d.%d.%d\n",
			ilits->protocol->ver >> 16, (ilits->protocol->ver >> 8) & 0xFF,
			ilits->protocol->ver & 0xFF);
	length += snprintf(buf+length, KTP_MAX_PRBUF_SIZE-length, "Driver version = %s\n", DRIVER_VERSION);

	return length;
}

static ssize_t kctp_fw_version_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	ILI_INFO("kctp_fw_version_store\n");

	return count;
}
static DEVICE_ATTR_RW(kctp_fw_version);


static ssize_t kctp_gesture_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	u32 length = 0;

	ILI_INFO("kctp_gesture_mode_show\n");

	ILI_INFO("gesture = %d\n", ilits->gesture);

	length += snprintf(buf+length, KTP_MAX_PRBUF_SIZE-length, "gesture_mode=%d\n", ilits->gesture);

	return length;
}

static ssize_t kctp_gesture_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	ILI_INFO("kctp_gesture_mode_store\n");
...
static DEVICE_ATTR_RW(kctp_gesture_mode);

static ssize_t kctp_charge_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	u32 length = 0;

	ILI_INFO("%s\n", __func__);

	ILI_INFO("kc_usb_plug_status = %d\n", ilits->kc_usb_plug_status);

	length += snprintf(buf+length, KTP_MAX_PRBUF_SIZE-length, "kc_usb_plug_status=%d\n", ilits->kc_usb_plug_status);

	return length;
}

static ssize_t kctp_charge_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int ret = 0;
	...
	if(buf[0] == '0'){
		ilits->kc_usb_plug_status = KTP_CHARGE_MODE_USB_PLUGIN;
	}else if(buf[0] == '1'){
		ilits->kc_usb_plug_status = KTP_CHARGE_MODE_NONE;
	}else if(buf[0] == '2'){
		ilits->kc_usb_plug_status = KTP_CHARGE_MODE_WIRELESS;
	}else{
		ILI_INFO("invalid value!\n");
	}
	ILI_INFO("kc_usb_plug_status = %d\n", ilits->kc_usb_plug_status);

	ret = ili_ic_func_ctrl("plug", ilits->kc_usb_plug_status);
	if (ret < 0) {
...
static DEVICE_ATTR_RW(kctp_charge_mode);

static ssize_t kctp_event_ctrl_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	u32 length = 0;

	ILI_INFO("%s\n", __func__);

	ILI_INFO("kc_event_ctrl = %d\n", ilits->kc_event_ctrl);

	length += snprintf(buf+length, KTP_MAX_PRBUF_SIZE-length, "kc_event_ctrl=%d\n", ilits->kc_event_ctrl);

	return length;
}

static ssize_t kctp_event_ctrl_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	ILI_INFO("%s\n", __func__);

	if(buf[0] == '1'){
		ilits->kc_event_ctrl = 1;
	}else{
		ilits->kc_event_ctrl = 0;
	}
	ILI_INFO("kc_event_ctrl = %d\n", ilits->kc_event_ctrl);

	return count;
}
static DEVICE_ATTR_RW(kctp_event_ctrl);


static struct attribute *ilitek_dev_syscomd_atts[] = {
	&dev_attr_kctp_fw_version.attr,
	&dev_attr_kctp_gesture_mode.attr,
	&dev_attr_kctp_charge_mode.attr,
	&dev_attr_kctp_event_ctrl.attr,
	NULL
};

static const struct attribute_group ilitek_dev_syscomd_atts_group = {
	.attrs = ilitek_dev_syscomd_atts,
};

static const struct attribute_group *ilitek_dev_attr_groups[] = {
	&ilitek_dev_syscomd_atts_group,
	NULL
};
...
#ifdef KCTP_CUST
int kctp_ilitek_plat_early_suspend(void *pdata, int order)
{

	ILI_DBG("touch order case=%d\n", order);
	switch(order){
	case 0:
		break;
	case 1:
...
int kctp_ilitek_plat_late_resume(void *pdata, int order)
{
	ILI_DBG("touch order case=%d\n", order);
	switch(order){
	case 0:
...
#else
#ifdef KCTP_CUST
	ILI_INFO("create sysfs node +\n");
	ili_sysfs_add_device(ilits->dev);
	if (sysfs_create_link(NULL, &ilits->dev->kobj, "touchscreen") < 0)
		ILI_INFO("Failed to sysfs node create link!\n");
	ILI_INFO("create sysfs node -\n");
#endif /* KCTP_CUST */
...
#ifdef KCTP_CUST
	kctp_ctrl_pinctrl_init();
	kctp_ctrl_pinctrl_select(true);
	kdisp_touch_set_info(ilits);
