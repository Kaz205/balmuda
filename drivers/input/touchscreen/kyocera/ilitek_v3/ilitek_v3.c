...
#ifdef KCTP_CUST
int kctp_ctrl_pinctrl_init(void)
{
	int retval = 0;

	pr_err("[KCTP]%s\n", __func__);

	ilits->ts_pinctrl = devm_pinctrl_get(ilits->dev);
	if (IS_ERR_OR_NULL(ilits->ts_pinctrl)) {
		pr_debug("%s: Target does not use pinctrl\n", __func__);
		retval = PTR_ERR(ilits->ts_pinctrl);
		ilits->ts_pinctrl = NULL;
		return retval;
	}

	ilits->gpio_state_active
		= pinctrl_lookup_state(ilits->ts_pinctrl, "pmx_ts_active");
	if (IS_ERR_OR_NULL(ilits->gpio_state_active)) {
		pr_debug("%s: Can not get ts default pinstate\n", __func__);
		retval = PTR_ERR(ilits->gpio_state_active);
		ilits->ts_pinctrl = NULL;
		return retval;
	}

	ilits->gpio_state_suspend
		= pinctrl_lookup_state(ilits->ts_pinctrl, "pmx_ts_sleep");
	if (IS_ERR_OR_NULL(ilits->gpio_state_suspend)) {
		pr_debug("%s: Can not get ts sleep pinstate\n", __func__);
		retval = PTR_ERR(ilits->gpio_state_suspend);
		ilits->ts_pinctrl = NULL;
		return retval;
	}
	return 0;
...
int kctp_ctrl_pinctrl_select(bool onoff)
{
	struct pinctrl_state *pins_state;
	int ret;

	pr_err("[KCTP]%s onoff=%d\n", __func__, onoff);

	pins_state = onoff ? ilits->gpio_state_active
					: ilits->gpio_state_suspend;
	if (!IS_ERR_OR_NULL(pins_state)) {
		ret = pinctrl_select_state(ilits->ts_pinctrl, pins_state);
		if (ret) {
			pr_err("[KCTP]%s can not set %s pins\n", __func__, onoff ? "pmx_ts_active" : "pmx_ts_sleep");
			return ret;
		}

	} else {
		pr_err("[KCTP]%s not a valid '%s' pinstate\n", __func__, onoff ? "pmx_ts_active" : "pmx_ts_sleep");
	}
...
#ifdef KCTP_CUST
			if (ili_reset_ctrl(TP_DUMMY_RST_FLAG_ONLY) < 0)
...
#ifdef KCTP_CUST
	if(ilits->kc_event_ctrl){
		ILI_ERR("Touch Event Disable. Debug mode on.\n");
		pid = P5_X_DEBUG_PACKET_ID;
	}
#endif /* KCTP_CUST */
...
int ili_reset_ctrl(int mode)
{
	...
	case TP_HW_RST_ONLY:
		...
		break;
#ifdef KCTP_CUST
	case TP_DUMMY_RST_FLAG_ONLY:
		ILI_INFO("TP WAKEUP RST\n");
		ilits->pll_clk_wakeup = true;
		break;
#endif /* KCTP_CUST */
	default:
