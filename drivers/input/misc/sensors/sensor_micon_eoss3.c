...
#define SENSOR_MICON_WAKELOCK_STATUS        (SIGN_MOTION_DETECTED | PD_STEP | VC_DETECTED | \
                                             ...
                                             WUG_DETECT | FDM_DETECT | FTO_DETECT)
...
static DEFINE_MUTEX(accuc_report_event_now_mutex);
...
static struct acc_uncalib                    last_accuc_data     = {0};
...
static struct acceleration          acc_offsets       = {0};
...
static struct timestamp_filter_val accuc_ts      = {0};
...
static uint8_t g_acc_current_drange     = ACC_DYNAMIC_RANGE_INVALID;
static uint8_t g_gyro_current_drange    = GYRO_DYNAMIC_RANGE_INVALID;
...
static const uint8_t batch_id_to_tbl_idx[] =
{
    ...
    [SENSOR_BATCH_ID_ACC_UNCAL_RAW] = SENSOR_BATCH_TBL_ACC_UNCAL_RAW,
    [SENSOR_BATCH_ID_ACC_UNCAL_OFS] = SENSOR_BATCH_TBL_ACC_UNCAL_OFS,
...
static const uint8_t tbl_idx_to_size[] =
{
    ...
    [SENSOR_BATCH_TBL_ACC_UNCAL_RAW]  = SENSOR_BATCH_SIZE_ACC_UNCAL_RAW,
    [SENSOR_BATCH_TBL_ACC_UNCAL_OFS]  = SENSOR_BATCH_SIZE_ACC_UNCAL_OFS,
...
static const uint8_t tbl_idx_to_nousesize[] =
{
    ...
    [SENSOR_BATCH_TBL_ACC_UNCAL_RAW]  = SENSOR_BATCH_NOUSE_SIZE_ACC_UNCAL_RAW,
    [SENSOR_BATCH_TBL_ACC_UNCAL_OFS]  = SENSOR_BATCH_NOUSE_SIZE_ACC_UNCAL_OFS,
...
static const uint8_t tbl_idx_to_smid[] =
{
    ...
    [SENSOR_BATCH_TBL_ACC_UNCAL_RAW] = SHID_ACCELEROMETER_UNCALIBRATED,
    [SENSOR_BATCH_TBL_ACC_UNCAL_OFS] = SHID_ACCELEROMETER_UNCALIBRATED,
...
static const uint8_t sensor_type_to_tbl_idx[] =
{
    ...
    [SENSOR_ACC_UNCAL]      = SENSOR_BATCH_TBL_ACC_UNCAL_OFS,
...
static const uint8_t sensor_type_to_smid[] =
{
    ...
    [SENSOR_ACC_UNCAL]      = SHID_ACCELEROMETER_UNCALIBRATED,
...
static int32_t pre_sensor_enable_for_ffe_check(void);
...
static int32_t kcmot_wakeupgesture_enable(bool enable);
static int32_t kcmot_get_facedown_data(struct kc_motion_facedown_data *arg_Facedown);
static int32_t kcmot_facedown_enable(bool enable);
//static int32_t kcmot_get_flick_data(struct kc_motion_flick_data *arg_Flick);
static int32_t kcmot_flick_enable(bool enable);
...
static int32_t acc_set_dynamic_range(uint8_t input_drange);
static int32_t acc_get_dynamic_range(uint8_t* output_drange);
...
static int32_t gyro_set_dynamic_range(uint8_t input_drange);
static int32_t gyro_get_dynamic_range(uint8_t* output_drange);
...
static int32_t get_dynamic_range(uint8_t *val, const uint8_t sensortype);
...
static void get_sns_value(enum sensor_e_type type, void *outdata)
{
    ...
    switch (type){
        ...
        case SENSOR_MAG:
            ...
            break;
        case SENSOR_GYRO:
            last_gyro_data.x = CULC_GYRO_VAL_DIGIT_2_0P1DPS(last_gyro_data.x);
            last_gyro_data.y = CULC_GYRO_VAL_DIGIT_2_0P1DPS(last_gyro_data.y);
            last_gyro_data.z = CULC_GYRO_VAL_DIGIT_2_0P1DPS(last_gyro_data.z);
            memcpy(outdata, &last_gyro_data, sizeof(last_gyro_data));
            break;
        case SENSOR_ACC_UNCAL:
            memcpy(outdata, &last_accuc_data, sizeof(last_accuc_data));
            break;
        case SENSOR_MAG_UNCAL:
            memcpy(outdata, &last_maguc_data, sizeof(last_maguc_data));
            break;
        case SENSOR_GYRO_UNCAL:
            last_gyrouc_data.x = CULC_GYROUC_VAL_DIGIT_2_0P1DPS(last_gyrouc_data.x);
            last_gyrouc_data.y = CULC_GYROUC_VAL_DIGIT_2_0P1DPS(last_gyrouc_data.y);
            last_gyrouc_data.z = CULC_GYROUC_VAL_DIGIT_2_0P1DPS(last_gyrouc_data.z);
...
static void backup_sns_value(void)
{
    ...
    prv_gyro.x = CULC_GYRO_VAL_DIGIT_2_0P1DPS(last_gyro_data.x);
    prv_gyro.y = CULC_GYRO_VAL_DIGIT_2_0P1DPS(last_gyro_data.y);
    prv_gyro.z = CULC_GYRO_VAL_DIGIT_2_0P1DPS(last_gyro_data.z);
    SENSOR_D_LOG("end");
}

static int32_t convert_acc_unit(int32_t digit)
{
    int32_t sensor_unit = digit;
    int32_t factor = 0;

    SENSOR_D_LOG("start");
#ifndef USE_UNIT_CONVERT
    SENSOR_D_LOG("need not convert.");
    return sensor_unit;
#endif
    switch(g_acc_current_drange){
        case ACC_DYNAMIC_RANGE_16G:
        case ACC_DYNAMIC_RANGE_8G:
        case ACC_DYNAMIC_RANGE_4G:
        case ACC_DYNAMIC_RANGE_2G:
            factor = acc_unitconv_factor[g_acc_current_drange];
            break;
        default:
            factor =acc_unitconv_factor[ACC_DYNAMIC_RANGE_INVALID];
            break;
    }
    sensor_unit = CULC_ACC_VAL_DIGIT_2_MG(digit, factor);
    SENSOR_D_LOG("Value %d[ms], DynamicRange[0x%02x], factor[%d]",
                        sensor_unit, g_acc_current_drange, factor);
    SENSOR_D_LOG("end.");
    return sensor_unit;
}

static int32_t convert_gyro_unit(int32_t digit)
{
    int32_t sensor_unit = digit;
    int32_t factor;
    SENSOR_D_LOG("start");
    ...
    return sensor_unit;
#endif
    switch(g_gyro_current_drange){
        case GYRO_DYNAMIC_RANGE_125DPS:
        case GYRO_DYNAMIC_RANGE_245DPS:
        case GYRO_DYNAMIC_RANGE_500DPS:
        case GYRO_DYNAMIC_RANGE_1000DPS:
        case GYRO_DYNAMIC_RANGE_2000DPS:
            factor = gyro_unitconv_factor[g_gyro_current_drange];
            break;
        default:
            factor = gyro_unitconv_factor[GYRO_DYNAMIC_RANGE_INVALID];
            break;
    }
    sensor_unit = digit / factor;
    SENSOR_D_LOG("Value %d[digit], DynamicRange[0x%02x], factor[%d]",
                    sensor_unit, g_gyro_current_drange, factor);
    SENSOR_D_LOG("end.");
    return sensor_unit;
}

static int32_t convert_accuc_unit(int32_t digit)
{
    SENSOR_N_LOG("start");
    SENSOR_N_LOG("end.");
    return convert_acc_unit(digit);
}

static int32_t convert_gyrouc_unit(int32_t digit)
{
    SENSOR_D_LOG("start");
    SENSOR_D_LOG("end.");
    return convert_gyro_unit(digit);
}
static int32_t convert_mag_unit(int32_t digit)
{
    int32_t sensor_unit = digit;
    SENSOR_D_LOG("start");
#ifdef USE_UNIT_CONVERT
    sensor_unit = CULC_MAG_VAL_DIGIT_2_0P1UT(digit);
#endif /* USE_UNIT_CONVERT */
    SENSOR_D_LOG("end.");
    return sensor_unit;
}

static int32_t convert_maguc_unit(int32_t digit)
{
    SENSOR_D_LOG("start");
    SENSOR_D_LOG("end.");
    return convert_mag_unit(digit);
}

static int32_t convert_press_unit(int32_t digit)
{
    int32_t sensor_unit = digit;
    SENSOR_D_LOG("start");
#if defined(USE_UNIT_CONVERT) && defined(CONFIG_USE_SENSOR_LPS22HH)
    sensor_unit = CULC_PRESS_VAL_DIGIT_2_PA(digit);
#endif /* USE_UNIT_CONVERT */
    SENSOR_D_LOG("end.");
    return sensor_unit;
}
...
static void read_all_sensor_value(void)
{
    ...
    last_acc_data.outX = convert_acc_unit((int32_t)x);
    last_acc_data.outY = convert_acc_unit((int32_t)y);
    last_acc_data.outZ = convert_acc_unit((int32_t)z);
    ...
    z = s_data.mag_zA;
    A = s_data.mag_zA >> 16;
    last_mag_data.x = convert_mag_unit((int32_t)x);
    last_mag_data.y = convert_mag_unit((int32_t)y);
    last_mag_data.z = convert_mag_unit((int32_t)z);
    ...
    y = s_data.gyro_xy >> 16;
    z = s_data.gyro_z;
    last_gyro_data.x = convert_gyro_unit((int32_t)x);
    last_gyro_data.y = convert_gyro_unit((int32_t)y);
    last_gyro_data.z = convert_gyro_unit((int32_t)z);

    //Pressure
    Pl = s_data.press_Pa_L;
    Ph = s_data.press_Pa_H;
    last_press_data.pressure = convert_press_unit(Pl);
    ...
    //AccelerometerFieldUncalibrated
    x = s_data.accel_xy;
    y = s_data.accel_xy >> 16;
    z = s_data.accel_z & 0xFFFF;
    o_x = s_data.accel_calib_offs_xy;
    o_y = s_data.accel_calib_offs_xy >> 16;
    o_z = s_data.accel_calib_offs_z & 0xFFFF;

    memset(&acc_offsets,0, sizeof(acc_offsets));
    acc_get_ac_offsets(&acc_offsets);

    last_accuc_data.x       = convert_accuc_unit((int32_t)x) + acc_offsets.nX;
    last_accuc_data.y       = convert_accuc_unit((int32_t)y) + acc_offsets.nY;
    last_accuc_data.z       = convert_accuc_unit((int32_t)z) + acc_offsets.nZ;
    last_accuc_data.cal_x = (int32_t)acc_offsets.nX;
    last_accuc_data.cal_y = (int32_t)acc_offsets.nY;
    last_accuc_data.cal_z = (int32_t)acc_offsets.nZ;
    ...
    last_maguc_data.x       = convert_maguc_unit((int32_t)x) + convert_maguc_unit(o_x);
    last_maguc_data.y       = convert_maguc_unit((int32_t)y) + convert_maguc_unit(o_y);
    last_maguc_data.z       = convert_maguc_unit((int32_t)z) + convert_maguc_unit(o_z);
    last_maguc_data.cal_x   = convert_maguc_unit((int32_t)o_x);
    last_maguc_data.cal_y   = convert_maguc_unit((int32_t)o_y);
    last_maguc_data.cal_z   = convert_maguc_unit((int32_t)o_z);
    ...
    last_gyrouc_data.x       = convert_gyrouc_unit((int32_t)x) + gyro_dynamic_offset[0];
    last_gyrouc_data.y       = convert_gyrouc_unit((int32_t)y) + gyro_dynamic_offset[1];
    last_gyrouc_data.z       = convert_gyrouc_unit((int32_t)z) + gyro_dynamic_offset[2];
...
static void accuc_clear_filt_coff(void){
	accuc_val_clear_flg = true;
}

static void accuc_clear_filt_coff_exe(bool flg){
	if (flg || (accuc_ts.base_set_val != accuc_base_set_val_rcv)) {
	    memset(&prv_acc,0, sizeof(prv_acc));
	    memset(&acc_rel,0, sizeof(acc_rel));
	    memset(&acc_val,0, sizeof(acc_val));
	    memset(&acc_hys_val,0, sizeof(acc_hys_val));
	    memset(&accuc_ts,0, sizeof(accuc_ts));
	    SENSOR_D_LOG("Accuc_filt_coff_clear_done!");
        accuc_val_clear_flg = false;
	}
}

static void accuc_part_clear_filt_coff(void){
    memset(&acc_rel,0, sizeof(acc_rel));
    memset(&acc_val,0, sizeof(acc_val));
    memset(&acc_hys_val,0, sizeof(acc_hys_val));
    SENSOR_D_LOG("Accuc_filt_part_coff_clear!");
}
    ...
    if (ts->base_rel >= th_Rel){
        ...
        if(TmpDelta > ts->base_val) {
            ...
            if (TmpDelta  <= (ts->base_val * (-1 * unaccept_num))) {
                ...
                ts->luck_sample_num = abs(TmpDelta) / ts->base_val;
...
static int32_t get_sensor_batch_processing_param(uint32_t logging_enable)
{
    ...
    output_sns_param(&param, IS_ACC_UNCAL_EN(logging_enable),   STC_ACCEL_UNCALIBRATED_BATCHING );
    SENSOR_D_LOG("end");
    return param;
}
#endif
...
{
	int32_t ret;
	struct sensor_ctrl_param_output_str param_old;
	struct sensor_ctrl_param_output_str param;
	memset(&param_old, 0, sizeof(param_old));
	memset(&param, 0, sizeof(param));

	SENSOR_D_LOG("start");

	param.period_FFETimer = 1;             //FFE Timer_F "1"ms
	param.sensor.period_sensor_task = 1;   //Sns TaskFrq "1"ms
	param.sensor.period_acc = 10;          //Sns PollFrq Acc"10"ms / Acc ODR
	param.sensor.period_mag = 480;         //Sns PollFrq Mag"480"ms / Mag ODR
	param.sensor.period_gyro = 480;        //Sns PollFrq Gyr"480"ms / Gyr ODR
	param.sensor.period_pressure = 480;    //Sns PollFrq Prs"480"ms / Prs ODR
	param.sensor.period_app_task = 240;    //FFa TaskFrq "240"ms
	param.sensor.period_app_task = 240;    //M4a TaskFrq "240"ms
	param.sensor.period_fusion = 100;      //CalcFrq Rot"100"ms Geo"100"ms Gam"100"ms Ori"100"ms Grv"100"ms Lin"100"ms MagCal"100"ms
	param.sensor.period_fusion_task = 100; //FFf TaskFrq "100"ms
	param.sensor.period_fusion_task = 100; //M4f TaskFrq "100"ms
	param.sensor.acc_param[4] = 0;         //Acc PowMode "LPM"
	param.sensor.enable = 0x00000001;      //Sns EnDvPol Acc"En" Mag"Dis" Gyr"Dis" Prs"Dis"
	                                       //Fsn EnSwSns Rot"Dis" Geo"Dis" Gam"Dis" Ori"Dis" Grv"Dis" Lin"Dis" MagCal"Dis" StpCnt"Dis" StpDet"Dis" SigMot"Dis" / FFE EnTimer "En"

	param.sensor.task_exec[0] = 1; //Sns EnTask "En"
	param.sensor.task_exec[2] = 0; //FFf EnTask "Dis" / M4f EnTask "Dis"
	param.sensor.task_exec[1] = 0; //FFa EnTask "Dis" / M4a EnTask "Dis"

	ret = send_sensor_ctrl(&param_old, &param, false);
	if(ret != SNS_RC_OK){
		SENSOR_ERR_LOG("Error:send_sensor_ctrl enable");
		goto exit;
	}

	msleep(20);
	param.sensor.period_acc = 480; //Sns PollFrq Acc"480"ms
	param.sensor.task_exec[0] = 0; //Sns EnTask  "Dis"
	param.sensor.enable = 0;       //FFE EnTimer "Dis"

	ret = send_sensor_ctrl(&param_old, &param, false);
	if(ret != SNS_RC_OK){
		SENSOR_ERR_LOG("Error:send_sensor_ctrl disable");
		goto exit;
	}
...
#endif
#ifdef CONFIG_INPUT_SENSOR_UNCAL_ACCELEROMETER
    case SENSOR_BATCH_ID_ACC_UNCAL_RAW:
    case SENSOR_BATCH_ID_ACC_UNCAL_OFS:
#endif
...
static int16_t combine_two_8bit_to_s16bit(uint8_t upper, uint8_t lower)
{
...
static int32_t iio_report_events(uint8_t *batch_data, uint32_t len)
{
    ...
    int32_t  size;
    uint8_t  nousesize = 0;
    uint8_t  acc_uncal_raw[6] = {0}; /* X,Y,Z */
    ...
    while (remain > 0)
    {
        ...
        case SENSOR_BATCH_TBL_ACC:
            x       = combine_two_8bit_to_s16bit(*(buf+nousesize+2), *(buf+nousesize+1));
            x       = (int16_t)convert_acc_unit((int32_t)x);
            y       = combine_two_8bit_to_s16bit(*(buf+nousesize+4), *(buf+nousesize+3));
            y       = (int16_t)convert_acc_unit((int32_t)y);
            z       = combine_two_8bit_to_s16bit(*(buf+nousesize+6), *(buf+nousesize+5));
            z       = (int16_t)convert_acc_unit((int32_t)z);
            data[0] = (uint8_t)x & 0x00FF;          //X_L
            data[1] = (uint8_t)((x >> 8) & 0x00FF); //X_H
            data[2] = (uint8_t)y & 0x00FF;          //Y_L
            data[3] = (uint8_t)((y >> 8) & 0x00FF); //Y_H
            data[4] = (uint8_t)z & 0x00FF;          //Z_L
            data[5] = (uint8_t)((z >> 8) & 0x00FF); //Z_H
        ...
        case SENSOR_BATCH_TBL_MAG:
            x       = combine_two_8bit_to_s16bit(*(buf+nousesize+3), *(buf+nousesize+2));
            x       = (int16_t)convert_mag_unit((int32_t)x);
            y       = combine_two_8bit_to_s16bit(*(buf+nousesize+5), *(buf+nousesize+4));
            y       = (int16_t)convert_mag_unit((int32_t)y);
            z       = combine_two_8bit_to_s16bit(*(buf+nousesize+7), *(buf+nousesize+6));
            z       = (int16_t)convert_mag_unit((int32_t)z);
            data[0] = (uint8_t)x & 0x00FF;          //X_L
            data[1] = (uint8_t)((x >> 8) & 0x00FF); //X_H
            data[2] = (uint8_t)y & 0x00FF;          //Y_L
            data[3] = (uint8_t)((y >> 8) & 0x00FF); //Y_H
        ...
        case SENSOR_BATCH_TBL_GYRO:
            x       = combine_two_8bit_to_s16bit(*(buf+nousesize+2), *(buf+nousesize+1));
            x       = (int16_t)convert_gyro_unit((int32_t)x);
            y       = combine_two_8bit_to_s16bit(*(buf+nousesize+4), *(buf+nousesize+3));
            y       = (int16_t)convert_gyro_unit((int32_t)y);
            z       = combine_two_8bit_to_s16bit(*(buf+nousesize+6), *(buf+nousesize+5));
            z       = (int16_t)convert_gyro_unit((int32_t)z);
            data[0] = (uint8_t)x & 0x00FF;          //X_L
            data[1] = (uint8_t)((x >> 8) & 0x00FF); //X_H
            data[2] = (uint8_t)y & 0x00FF;          //Y_L
            data[3] = (uint8_t)((y >> 8) & 0x00FF); //Y_H
        ...
        case SENSOR_BATCH_TBL_ACC_UNCAL_RAW:
            x       = combine_two_8bit_to_s16bit(*(buf+nousesize+2), *(buf+nousesize+1));
            x       = (int16_t)convert_acc_unit((int32_t)x);
            y       = combine_two_8bit_to_s16bit(*(buf+nousesize+4), *(buf+nousesize+3));
            y       = (int16_t)convert_acc_unit((int32_t)y);
            z       = combine_two_8bit_to_s16bit(*(buf+nousesize+6), *(buf+nousesize+5));
            z       = (int16_t)convert_acc_unit((int32_t)z);
            acc_uncal_raw[0] = (uint8_t)x & 0x00FF;          //X_L
            acc_uncal_raw[1] = (uint8_t)((x >> 8) & 0x00FF); //X_H
            acc_uncal_raw[2] = (uint8_t)y & 0x00FF;          //Y_L
            acc_uncal_raw[3] = (uint8_t)((y >> 8) & 0x00FF); //Y_H
            acc_uncal_raw[4] = (uint8_t)z & 0x00FF;          //Z_L
            acc_uncal_raw[5] = (uint8_t)((z >> 8) & 0x00FF); //Z_H
            break;

        case SENSOR_BATCH_TBL_ACC_UNCAL_OFS:
            memcpy(data, acc_uncal_raw, sizeof(acc_uncal_raw));
            x       = combine_two_8bit_to_s16bit(*(buf+nousesize+2), *(buf+nousesize+1));
            x       = (int16_t)convert_acc_unit((int32_t)x);
            y       = combine_two_8bit_to_s16bit(*(buf+nousesize+4), *(buf+nousesize+3));
            y       = (int16_t)convert_acc_unit((int32_t)y);
            z       = combine_two_8bit_to_s16bit(*(buf+nousesize+6), *(buf+nousesize+5));
            z       = (int16_t)convert_acc_unit((int32_t)z);
            data[6] = (uint8_t)x & 0x00FF;          //X_L
            data[7] = (uint8_t)((x >> 8) & 0x00FF); //X_H
            data[8] = (uint8_t)y & 0x00FF;          //Y_L
            data[9] = (uint8_t)((y >> 8) & 0x00FF); //Y_H
            data[10] = (uint8_t)z & 0x00FF;          //Z_L
            data[11] = (uint8_t)((z >> 8) & 0x00FF); //Z_H
            timestamp = convert_host_sensortimestamp(sensor_ts.ts, SENSOR_TASK);
            sns_iio_report_event(smid, data, (size+sizeof(acc_uncal_raw)), timestamp, 0);
            break;
        ...
        case SENSOR_BATCH_TBL_MAG_UNCAL_RAW:
            ...
            z       = combine_two_8bit_to_s16bit(*(buf+nousesize+7), *(buf+nousesize+6));
            z       = (int16_t)convert_mag_unit((int32_t)z);
            mag_uncal_raw[0] = (uint8_t)x & 0x00FF;          //magX_raw_L
            mag_uncal_raw[1] = (uint8_t)((x >> 8) & 0x00FF); //magX_raw_H
            mag_uncal_raw[2] = (uint8_t)y & 0x00FF;          //magY_raw_L
            mag_uncal_raw[3] = (uint8_t)((y >> 8) & 0x00FF); //magY_raw_H
            mag_uncal_raw[4] = (uint8_t)z & 0x00FF;          //magZ_raw_L
            mag_uncal_raw[5] = (uint8_t)((z >> 8) & 0x00FF); //magZ_raw_H
        ...
        case SENSOR_BATCH_TBL_MAG_UNCAL_OFS:
            ...
            z        = combine_two_8bit_to_s16bit(*(buf+nousesize+6), *(buf+nousesize+5));
            z        = (int16_t)convert_mag_unit((int32_t)z);
            data[7]  = (uint8_t)x & 0x00FF;          //magX_ofs_L
            data[8]  = (uint8_t)((x >> 8) & 0x00FF); //magX_ofs_H
            data[9]  = (uint8_t)y & 0x00FF;          //magY_ofs_L
            data[10] = (uint8_t)((y >> 8) & 0x00FF); //magY_ofs_H
            data[11] = (uint8_t)z & 0x00FF;          //magZ_ofs_L
            data[12] = (uint8_t)((z >> 8) & 0x00FF); //magZ_ofs_H
            timestamp = convert_host_sensortimestamp(sensor_ts.ts, SENSOR_TASK);
        ...
        case SENSOR_BATCH_TBL_GYRO_UNCAL_RAW:
            ...
            z       = combine_two_8bit_to_s16bit(*(buf+nousesize+6), *(buf+nousesize+5));
            z       = (int16_t)convert_gyro_unit((int32_t)z);
            gyro_uncal_raw[0] = (uint8_t)x & 0x00FF;          //gyroX_raw_L
            gyro_uncal_raw[1] = (uint8_t)((x >> 8) & 0x00FF); //gyroX_raw_H
            gyro_uncal_raw[2] = (uint8_t)y & 0x00FF;          //gyroY_raw_L
            gyro_uncal_raw[3] = (uint8_t)((y >> 8) & 0x00FF); //gyroY_raw_H
            gyro_uncal_raw[4] = (uint8_t)z & 0x00FF;          //gyroZ_raw_L
            gyro_uncal_raw[5] = (uint8_t)((z >> 8) & 0x00FF); //gyroZ_raw_H
            break;
        ...
        case SENSOR_BATCH_TBL_GYRO_UNCAL_OFS:
            ...
            z       = combine_two_8bit_to_s16bit(*(buf+nousesize+6), *(buf+nousesize+5));
            z       = (int16_t)convert_gyro_unit((int32_t)z);
            data[6]  = (uint8_t)x & 0x00FF;          //gyroX_ofs_L
            data[7]  = (uint8_t)((x >> 8) & 0x00FF); //gyroX_ofs_H
            data[8]  = (uint8_t)y & 0x00FF;          //gyroY_ofs_L
            data[9]  = (uint8_t)((y >> 8) & 0x00FF); //gyroY_ofs_H
            data[10] = (uint8_t)z & 0x00FF;          //gyroZ_ofs_L
            data[11] = (uint8_t)((z >> 8) & 0x00FF); //gyroZ_ofs_H
            timestamp = convert_host_sensortimestamp(sensor_ts.ts, SENSOR_TASK);
            sns_iio_report_event(smid, data, (size+sizeof(gyro_uncal_raw)), timestamp, 0);
            break;

        case SENSOR_BATCH_TBL_PRESSURE:
            p_in = *(buf+nousesize+4);
            p_in = (p_in << 8) | *(buf+nousesize+3);
            p_in = (p_in << 8) | *(buf+nousesize+2);
            p_in = (p_in << 8) | *(buf+nousesize+1);
            p_out = convert_press_unit(p_in);
            data[0] = p_out & 0x000000FF;         //X_L
            data[1] = (p_out >>  8) & 0x000000FF; //X_H
            data[2] = (p_out >> 16) & 0x000000FF; //Y_L
            data[3] = (p_out >> 24) & 0x000000FF; //Y_H
            timestamp = convert_host_sensortimestamp(sensor_ts.ts, SENSOR_TASK);
            sns_iio_report_event(smid, data, size, timestamp, 0);
            break;
...
static int iio_report_event_now(enum sensor_e_type type)
{
    ...
    case SENSOR_ACC_UNCAL:
        x = (int16_t)(last_accuc_data.x - last_accuc_data.cal_x);
        y = (int16_t)(last_accuc_data.y - last_accuc_data.cal_y);
        z = (int16_t)(last_accuc_data.z - last_accuc_data.cal_z);
        SENSOR_D_LOG("SENSOR_ACC_UNCAL acc uc        %d\t%d\t%d", x, y, z);

        mutex_lock(&accuc_report_event_now_mutex);

        accuc_clear_filt_coff_exe(accuc_val_clear_flg);

        if(no_noize_reduction_filter(x, y, z, SENSOR_ACC_UNCAL)){
            no_noize_reduction_filt_counter = INIT_NO_NR_FILT_TERM;
        }
        data[0] = (uint8_t)(x & 0x00ff);
        data[1] = (uint8_t)( (x >> 8) & 0x00ff);
        ...
        data[4] = (uint8_t)(z & 0x00ff);
        data[5] = (uint8_t)( (z >> 8) & 0x00ff);
        SENSOR_D_LOG("ACCUncal ts noize reduction.");
        now = apply_ts_noize_reduction_filter(now, &accuc_ts, accuc_base_set_val_rcv);

        o_x = (int16_t)last_accuc_data.cal_x;
        o_y = (int16_t)last_accuc_data.cal_y;
        o_z = (int16_t)last_accuc_data.cal_z;
        SENSOR_D_LOG("SENSOR_ACC_UNCAL acc uc(ofs)       %d\t%d\t%d", o_x, o_y, o_z);
    ...
    if (type == SENSOR_ACC_UNCAL){
        if (accuc_ts.luck_sample_num > 0) {
            for (i=0; i<accuc_ts.luck_sample_num; i++) {
                if (SNS_RC_OK == ret) {
                    sns_iio_report_event(
                            sensor_type_to_smid[type],
                            data,
                            EVENT_DATA_SIZE,
			                now + (accuc_ts.base_val * (i+1)),
                            0);
                }
            }
        }
        if (no_noize_reduction_filt_counter != 0) {
            accuc_part_clear_filt_coff();
        }
        mutex_unlock(&accuc_report_event_now_mutex);
    }
    if (type == SENSOR_GYRO){
        if(!sns_is_needed_gyro_sync()){
...
static int32_t kcmot_wakeupgesture_enable(bool enable)
{
    ...
    /* Send command */
    hc.command = prepare_host_command(APP, APP_WAKEUPGESTURE,
            APP_WAKEUPGESTURE__WUG_OnOff);
    hc.param_data[0] = 0x01           // CTRL_WUG_Main_Change
                     | (param << 8)   // CTRL_WUG_En_Function
                     | (param << 16)  // CTRL_WUG_En_Interrupt
                     | (0x00 << 24);
    hc.param_data[1] = 0x00;
    hc.param_data[2] = 0x00;
    hc.param_data_len = 4;
...
static int32_t kcmot_get_facedown_data(struct kc_motion_facedown_data *arg_Facedown)
{
    ...
    /* Send command */
    hc.command = prepare_host_command(APP, APP_FACEDOWNMUTE,
            APP_FACEDOWNMUTE__FDM_GetInfo);
    hc.param_data[0] = 0x00;
    hc.param_data[1] = 0x00;
    hc.param_data[2] = 0x00;
    hc.param_data[3] = 0x00;
    ...
    //CTRL_BU_State
    SENSOR_D_LOG("response 0[%d]",hc.response[0]);
    arg_Facedown->status = hc.response[0];
    mutex_unlock(&micon_hcres_mutex);
...
static int32_t kcmot_facedown_enable(bool enable)
{
    ...
    /* Send command */
    hc.command = prepare_host_command(APP, APP_FACEDOWNMUTE,
            APP_FACEDOWNMUTE__FDM_OnOff);
    hc.param_data[0] = 0x01           // CTRL_FDM_Main_Change
                     | (param << 8)   // CTRL_FDM_En_Function
                     | (param << 16)  // CTRL_FDM_En_Interrupt
                     | (0x00 << 24);
    hc.param_data[1] = 0x00;
    hc.param_data[2] = 0x00;
    hc.param_data_len = 4;
...
static int32_t kcmot_get_flick_data(struct kc_motion_flick_data *arg_Flick)
{
    ...
    /* Send command */
    hc.command = prepare_host_command(APP, APP_FLIPTURNON,
            APP_FLIPTURNON__FTO_GetInfo);
    hc.param_data[0] = 0x00;
    hc.param_data[1] = 0x00;
    hc.param_data[2] = 0x00;
    hc.param_data[3] = 0x00;
    ...
    //CTRL_BU_State
    SENSOR_D_LOG("response 0[%d]",hc.response[0]);
    arg_Flick->status = hc.response[0];
    mutex_unlock(&micon_hcres_mutex);
...
static int32_t kcmot_flick_enable(bool enable)
{
    ...
    /* Send command */
    hc.command = prepare_host_command(APP, APP_FLIPTURNON,
            APP_FLIPTURNON__FTO_OnOff);
    hc.param_data[0] = 0x01           // CTRL_FTO_Main_Change
                     | (param << 8)   // CTRL_FTO_En_Function
                     | (param << 16)  // CTRL_FTO_En_Interrupt
                     | (0x00 << 24);
    hc.param_data[1] = 0x00;
    hc.param_data[2] = 0x00;
    hc.param_data_len = 9;
...
static int32_t set_calib_val(const int16_t *c_val, const uint8_t accuracy, const enum ql_sensor_type sensortype)
{
    ...
    if( is_sns_connected(sensortype) ){
        ...
        if(sensortype == BAROMETER){
            ...
            hc.param_data_len = 7;
            hc.param_data[0] = ( (((uint32_t)c_val[1]) << 16) | ((uint32_t)c_val[0] & 0x0000FFFF) );
            hc.param_data[1] = ( (((uint32_t)accuracy) << 16) | ((uint32_t)c_val[2] & 0x0000FFFF) );
            hc.response_len = 0;
...
static int32_t InitSetting(void)
{
	...
	//Run FFE block once for FFE error check
	pre_sensor_enable_for_ffe_check();
...
static void accuc_set_ts_nrfilt_info(int32_t base_val)
{
    int64_t input_base_ns = base_val * 1000 * 1000;
    SENSOR_D_LOG("start");
    SENSOR_D_LOG("now setting accuc_ts.base_val[%lld], input_base_ns[%lld]",
                        accuc_ts.base_val, input_base_ns);
    if(accuc_ts.base_set_val != input_base_ns){
        SENSOR_D_LOG("change base val [%lld to %lld]",
                        accuc_ts.base_val, input_base_ns);
        accuc_base_set_val_rcv = input_base_ns;
        accuc_ts.init_done_flg = 0;
    }
    SENSOR_D_LOG("end");
}
...
static int8_t acc_set_selfchk_presetting(void)
{
    int8_t ret = SNS_RC_OK;
	SENSOR_D_LOG("start");
    acc_set_dynamic_range(ACC_DYNAMIC_RANGE_16G);
    SENSOR_D_LOG("end");
    return ret;
}
...
static int32_t acc_set_dynamic_range(uint8_t input_drange)
{
	int32_t ret = SNS_RC_OK;
	SENSOR_D_LOG("start");
    ret = set_dynamic_range(input_drange, ACCELEROMETER);
    if(ret == SNS_RC_OK){
        g_acc_current_drange = input_drange;
    }
	SENSOR_D_LOG("end");
    return ret;
}
...
static int32_t acc_get_dynamic_range(uint8_t* output_drange)
{
    int32_t ret = SNS_RC_OK;
	SENSOR_D_LOG("start");
    ret = get_dynamic_range(output_drange, ACCELEROMETER);
	SENSOR_D_LOG("end");
    return ret;
}
...
static int8_t gyro_set_selfchk_presetting(void)
{
    int8_t ret = SNS_RC_OK;
	SENSOR_D_LOG("start");
    gyro_set_dynamic_range(GYRO_DYNAMIC_RANGE_2000DPS);
    SENSOR_D_LOG("end");
    return ret;
}
...
static int32_t gyro_set_dynamic_range(uint8_t input_drange)
{
	int32_t ret = SNS_RC_OK;
	SENSOR_D_LOG("start");
    ret = set_dynamic_range(input_drange, GYROSCOPE);
    if(ret == SNS_RC_OK){
        g_gyro_current_drange = input_drange;
    }
	SENSOR_D_LOG("end");
    return ret;
}
...
static int32_t gyro_get_dynamic_range(uint8_t* output_drange)
{
    int32_t ret = SNS_RC_OK;
	SENSOR_D_LOG("start");
    ret = get_dynamic_range(output_drange, GYROSCOPE);
	SENSOR_D_LOG("end");
    return ret;
}
...
static int32_t set_dynamic_range(const uint8_t val, const uint8_t sensortype)
{
    ...
    if( is_sns_connected(sensortype) ){
        SENSOR_D_LOG("type[0x%02x], input dynamic range[0x%02x]",
                        sensortype, val);
        hc.command = prepare_host_command(SENSOR_DEVICE_CONTROL,
...
static int32_t get_dynamic_range(uint8_t *val, const uint8_t sensortype)
{
    ...
    if( is_sns_connected(sensortype) ){
        hc.command = prepare_host_command(SENSOR_DEVICE_CONTROL,
                sensortype, SDC_GET_DYNAMIC_RANGE);
        hc.response_len = 1;
        ret = qleoss3_hostcmd(&hc);
        *val = ((ret == QL_STATUS_OK) ? hc.response[0] : 0xFF);
    } else {
        SENSOR_ERR_LOG("skipped. sensor[0x%02x] is dis-connected.", sensortype);
    }
    SENSOR_D_LOG("end[%d]", ret);
...
#ifdef CONFIG_INPUT_SENSOR_ACCELEROMETER
	range_val = ACC_DYNAMIC_RANGE_4G;
	ret = acc_set_dynamic_range(range_val);
	if(ret != SNS_RC_OK){
...
#ifdef CONFIG_INPUT_SENSOR_GYROSCOPE
	range_val = GYRO_DYNAMIC_RANGE_2000DPS;
	ret = gyro_set_dynamic_range(range_val);
	if(ret != SNS_RC_OK){
...
static void irq_proc_priority3(uint64_t result)
{
    ...
    if(result & IRQ_PROC_PRIORITY3){
		if (result & MICON_STATE_IS_DEAD) {
			SENSOR_ERR_LOG("Micon dead is detected\n");
			sns_notify(SENSOR_EXT_PEDO, 99);
			return;
		}
...
static void irq_proc_priority6(uint64_t result)
{
    ...
    if(result & IRQ_PROC_PRIORITY6){
        ...
        if (result & WUG_DETECT) {
            SENSOR_D_LOG("WUG_DETECT\n");
            sns_notify(SENSOR_KC_MOTION_WAKEUPGESTURE,0);
        }

        if (result & FDM_DETECT) {
            SENSOR_D_LOG("FDM_DETECT\n");
            sns_notify(SENSOR_KC_MOTION_FACEDOWN,0);
        }

        if (result & FTO_DETECT) {
            SENSOR_D_LOG("FTO_DETECT\n");
            sns_notify(SENSOR_KC_MOTION_FLICK,0);
        }
...
void func_register_eoss3(struct sensor_micon_if_funcs *f)
{
    ...
    f->acc_func.set_dynamic_range   = acc_set_dynamic_range;
    f->acc_func.get_dynamic_range   = acc_get_dynamic_range;
    f->accuc_func.clear_filt_coff    = accuc_clear_filt_coff;
    f->accuc_func.set_ts_nrfilt_info = accuc_set_ts_nrfilt_info;
    ...
    f->gyro_func.set_dynamic_range  = gyro_set_dynamic_range;
    f->gyro_func.get_dynamic_range  = gyro_get_dynamic_range;
    ...
    f->kcmot_func.wakeupgesture_enable   = kcmot_wakeupgesture_enable;
    f->kcmot_func.facedown_enable   = kcmot_facedown_enable;
    f->kcmot_func.facedown_get_data = kcmot_get_facedown_data;
    f->kcmot_func.flick_enable      = kcmot_flick_enable;
