...
static ssize_t gyro_nrfilt_prm_show(struct device *dev,
    struct device_attribute *attr,
    char *buf);
static ssize_t gyro_dynamic_range_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf,
    size_t count);
static ssize_t gyro_dynamic_range_show(struct device *dev,
    struct device_attribute *attr,
    char *buf);
...
static DEVICE_ATTR(dynamic_range,
    S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP,
    gyro_dynamic_range_show,
    gyro_dynamic_range_store
);
    ...
    size_t count)
{
    ...
    sensor_com_mutex_lock();
    ret = sensor_set_dynamic_range(SENSOR_GYRO, val);
    sensor_com_mutex_unlock();
    SENSOR_D_LOG("end");
    return count;
}
    ...
    char *buf)
{
    ...
    SENSOR_D_LOG("start");
    sensor_com_mutex_lock();
    sensor_get_dynamic_range(SENSOR_GYRO, &val);
    sensor_com_mutex_unlock();
    SENSOR_D_LOG("end");
...
static ssize_t gyro_selfchk_exec(char* buf)
{
    ...
    sensor_enable( SENSOR_GYRO, &gyro_poll_info, false );
    sensor_set_selfchk_config(SENSOR_GYRO, SELFCHK_OFF);
