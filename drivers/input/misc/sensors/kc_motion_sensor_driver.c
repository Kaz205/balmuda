...
#define KCMOTION_FUNC_WAKEUPGESTURE    (0x0020)
#define KCMOTION_FUNC_FACEDOWN         (0x0040)
#define KCMOTION_FUNC_FLICK            (0x0080)

#define ABS_X_DUMMY_VAL                (-1)
#define ABS_X_BRINGUP_VAL              (1)
#define ABS_X_WAKEUP_VAL               (2)
#define ABS_X_FACEDOWN_VAL             (3)
#define ABS_X_FLICK_VAL                (4)
...
static ssize_t kc_motion_sensor_interrupt_kind_show(struct device *dev,
    struct device_attribute *attr,
    char *buf);
static ssize_t kc_motion_sensor_wakeupgesture_data_show(struct device *dev,
    struct device_attribute *attr,
    char *buf);
static ssize_t kc_motion_sensor_facedown_data_show(struct device *dev,
    struct device_attribute *attr,
    char *buf);
static ssize_t kc_motion_sensor_flick_data_show(struct device *dev,
    struct device_attribute *attr,
    char *buf);
...
static DEVICE_ATTR(kc_motion_wakeupgesture_data,
    S_IRUSR|S_IRGRP,
    kc_motion_sensor_wakeupgesture_data_show,
    NULL
);
static DEVICE_ATTR(kc_motion_facedown_data,
    S_IRUSR|S_IRGRP,
    kc_motion_sensor_facedown_data_show,
    NULL
);
static DEVICE_ATTR(kc_motion_flick_data,
    S_IRUSR|S_IRGRP,
    kc_motion_sensor_flick_data_show,
    NULL
);
...
static struct attribute *kc_motion_sensor_attributes[] = {
    ...
    &dev_attr_kc_motion_wakeupgesture_data.attr,
    &dev_attr_kc_motion_facedown_data.attr,
    &dev_attr_kc_motion_flick_data.attr,
    NULL
};
    ...
    char *buf)
{
    ...
    if( SENSOR_ON == sensor_get_status(SENSOR_KC_MOTION_WAKEUPGESTURE)) {
        enable |= KCMOTION_FUNC_WAKEUPGESTURE;
    }
    if( SENSOR_ON == sensor_get_status(SENSOR_KC_MOTION_FACEDOWN)) {
        enable |= KCMOTION_FUNC_FACEDOWN;
    }
    if( SENSOR_ON == sensor_get_status(SENSOR_KC_MOTION_FLICK)) {
        enable |= KCMOTION_FUNC_FLICK;
    }
    ret = sprintf(buf, "%x\n",enable);
    ...
    size_t count)
{
    ...
    if ((type & KCMOTION_FUNC_WAKEUPGESTURE) == KCMOTION_FUNC_WAKEUPGESTURE ){
        sensor_enable_with_suspend( SENSOR_KC_MOTION_WAKEUPGESTURE, NULL, (bool)enable );
    }

    if ((type & KCMOTION_FUNC_FACEDOWN) == KCMOTION_FUNC_FACEDOWN ){
        sensor_enable_with_suspend( SENSOR_KC_MOTION_FACEDOWN, NULL, (bool)enable );
    }
    if ((type & KCMOTION_FUNC_FLICK) == KCMOTION_FUNC_FLICK ){
        sensor_enable_with_suspend( SENSOR_KC_MOTION_FLICK, NULL, (bool)enable );
    }
    ...
    char *buf)
{
    ...
    get_ret = kc_motion_sensor_get_data( SENSOR_KC_MOTION_FACEDOWN, &read_data );
    if (  0 == get_ret ) {
        ret = sprintf( buf, "%d\n",
                       read_data.facedown_data.status );
        SENSOR_N_LOG("bringup status = %d",read_data.facedown_data.status);
    }
...
static int32_t kc_motion_sensor_get_data( enum sensor_e_type type, union sensor_motion_read_data_u *read_data )
{
    ...
    struct kc_motion_facedown_data stFacedown;
    ...
    switch(type) {
        ...
        case SENSOR_KC_MOTION_BRINGUP:
            ...
            break;
        case SENSOR_KC_MOTION_WAKEUPGESTURE:
            break;
        case SENSOR_KC_MOTION_FACEDOWN:
            sensor_com_mutex_lock();
            sns_get_kc_motion_facedown_data(&stFacedown);
            sensor_com_mutex_unlock();
            read_data->facedown_data.status = stFacedown.status;
            break;
        case SENSOR_KC_MOTION_FLICK:
            break;
        default:
...
static void kc_motion_sensor_set_input_params( struct input_dev *dev )
{
    ...
    set_bit(KEY_WAKEUP | EV_ABS, dev->evbit);
...
void kc_motion_sensor_wakeupgesture_interrupt( void )
{
    ...
    if ( SENSOR_ON == sensor_get_status(SENSOR_KC_MOTION_WAKEUPGESTURE) ) {
        tmp_kind = atomic_read(&kc_motion_interrupt_kind);
        tmp_kind |= KCMOTION_FUNC_WAKEUPGESTURE;
        atomic_set(&kc_motion_interrupt_kind,tmp_kind);
        atomic_set(&kc_motion_interrupt_flag,1);
        wake_up_interruptible(&kc_motion_sensor_p_queue);

        sensor_report_abs_current_time(kc_motion_sensor_input_info.dev);
        input_report_abs( kc_motion_sensor_input_info.dev, ABS_X, ABS_X_DUMMY_VAL );
        input_report_abs( kc_motion_sensor_input_info.dev, ABS_X, ABS_X_WAKEUP_VAL );
        input_sync( kc_motion_sensor_input_info.dev );

        sensor_enable_with_suspend( SENSOR_KC_MOTION_WAKEUPGESTURE, NULL, 0 );
    }
    SENSOR_N_LOG("end");
    return;
}
...
void kc_motion_sensor_facedown_interrupt( void )
{
    ...
    if ( SENSOR_ON == sensor_get_status(SENSOR_KC_MOTION_FACEDOWN) ) {
        tmp_kind = atomic_read(&kc_motion_interrupt_kind);
        tmp_kind |= KCMOTION_FUNC_FACEDOWN;
        atomic_set(&kc_motion_interrupt_kind,tmp_kind);
        atomic_set(&kc_motion_interrupt_flag,1);
        wake_up_interruptible(&kc_motion_sensor_p_queue);
        ...
        sensor_report_abs_current_time(kc_motion_sensor_input_info.dev);
        input_report_abs( kc_motion_sensor_input_info.dev, ABS_X, ABS_X_DUMMY_VAL );
        input_report_abs( kc_motion_sensor_input_info.dev, ABS_X, ABS_X_FACEDOWN_VAL );
        input_sync( kc_motion_sensor_input_info.dev );
    }
    SENSOR_N_LOG("end");
    return;
...
void kc_motion_sensor_flick_interrupt( void )
{
    ...
    if ( SENSOR_ON == sensor_get_status(SENSOR_KC_MOTION_FLICK) ) {
        ...
        sensor_report_abs_current_time(kc_motion_sensor_input_info.dev);
        input_report_abs( kc_motion_sensor_input_info.dev, ABS_X, ABS_X_DUMMY_VAL );
        input_report_abs( kc_motion_sensor_input_info.dev, ABS_X, ABS_X_FLICK_VAL );
        input_sync( kc_motion_sensor_input_info.dev );

        sensor_enable_with_suspend( SENSOR_KC_MOTION_FLICK, NULL, 0 );
    }
    SENSOR_N_LOG("end");
    return;
}
...
{
	input_report_key(kc_motion_sensor_input_info.dev, KEY_WAKEUP, 1);
	input_sync( kc_motion_sensor_input_info.dev );
	input_report_key(kc_motion_sensor_input_info.dev, KEY_WAKEUP, 0);
	input_sync( kc_motion_sensor_input_info.dev );
}
#endif
...
void kc_motion_sensor_driver_init(void)
{
    ...
    SENSOR_N_LOG("sensor_input_init()-->ret[%d] dev[%p]",
                  ret, kc_motion_sensor_input_info.dev );
