...
static struct of_device_id ps_als_of_match[] = {
	{ .compatible = RPR0521_I2C_NAME,},
	{ },
};
...
static u16 nv_proximity_detect[NUM_OF_COLOR] =
	{0x01F4, 0x01F4, 0x01F4, 0x01F4, 0x01F4};
...
static u16 nv_photosensor_th2[NUM_OF_COLOR] =
	{0x02B2, 0x02B2, 0x02B2, 0x02B2, 0x02B2};
...
static u16 nv_photosensor_a2[NUM_OF_COLOR] =
	{0x7530, 0x7530, 0x7530, 0x7530, 0x7530};
static u16 nv_photosensor_a3[NUM_OF_COLOR] =
	{0x951A, 0x951A, 0x951A, 0x951A, 0x951A};
...
static u16 nv_photosensor_b3[NUM_OF_COLOR] =
	{0x1A9A, 0x1A9A, 0x1A9A, 0x1A9A ,0x1A9A};
...
static irqreturn_t ps_irq_handler(int irq, void *dev_id)
{
	...
	queue_work(rohm_workqueue,&ps_als->ps_work);
	SENSOR_D_LOG("end");
	return (IRQ_HANDLED);
}
...
static int ps_als_suspend(struct device *dev)
{
	...
	flush_workqueue(rohm_workqueue);
	mutex_lock(&ps_als->control_lock);
...
#define ALS_IR_FLUORESCENT_TH       (293)
#define ALS_IR_LEDBULB_TH           (421)
#define ALS_IR_SOLAR_TH             (20000)
#define ALS_IR_CANDESCENT_TH        (590)
...
{
	SENSOR_D_LOG("start");
	queue_work(rohm_workqueue, &ps_als->ps_cal_work);
}
	...
	switch(type){
		case LIGHT_FLUORESCENT:
			if(ir >= ALS_IR_FLUORESCENT_TH){
				...
				SENSOR_ERR_LOG("Near the LightSource[%d], ir[%d]", LIGHT_FLUORESCENT, ir);
			}
			break;
		case LIGHT_LED_BULB:
		...
		case LIGHT_SOLAR:
			if(ir >= ALS_IR_SOLAR_TH){
				...
				SENSOR_ERR_LOG("Under direct Sunlight[%d], ir[%d]", LIGHT_SOLAR, ir);
			}
			break;
		case LIGHT_CANDESCENT:
...
static int8_t ps_calib_lightsrc_estimation(PS_ALS_DATA *ps_als, int32_t ratio)
{
	int8_t lsrc = -1;
	SENSOR_D_LOG("start");
	if( (0 <= ratio) && (ratio < nv_photosensor_th0[ps_als->color]) ){
...
static int32_t ps_calib_chk_lightsrc(PS_ALS_DATA *ps_als)
{
	...
	for(i = 0; i < ALS_ELEM_NUM; i++){
		...
		data[i].visible = (int)(read_data_buf.als_data0 / GAIN_TABLE[dev_val->gain].DATA0);
		data[i].ir      = (int)(read_data_buf.als_data1 / GAIN_TABLE[dev_val->gain].DATA1);
		if(data[i].ir > 0){
	...
	for(i = THROW_AWAY_NUM; i < (ALS_ELEM_NUM-THROW_AWAY_NUM); i++){
		ir_sum += (int32_t)data[i].ir;
		vi_sum += (int32_t)data[i].visible;
	}
	ir_ave = calc_ave(ir_sum, (ALS_ELEM_NUM-(THROW_AWAY_NUM*2)));
	vi_ave = calc_ave(vi_sum, (ALS_ELEM_NUM-(THROW_AWAY_NUM*2)));

	if(vi_ave == 0){
		SENSOR_ERR_LOG("ERROR! Visible ave is 0");
		return PS_CAL_ERR_OTHER;
	}
	ratio = (ir_ave * 1000) / vi_ave;

	lightsrc = ps_calib_lightsrc_estimation(ps_als, ratio);
	chk_result = ps_calib_chk_lighterr(lightsrc, ir_ave);
...
static int8_t ps_calib_chk_pscnt(PS_ALS_DATA *ps_als, uint16_t* ps_th_base)
{
	...
	for(i = 0; i < PS_ELEM_NUM; i++){
		...
		ps_cnt[i] = (int32_t)ps_als->ps_val.ps_data;
	}
...
static void ps_cal_func(struct work_struct *ps_cal_work)
{
	...
	bak_modectrl = ps_als_i2c_read_byte_data(ps_als->client, REG_MODECONTROL);
	if(unlikely(bak_modectrl < 0)) {
		SENSOR_ERR_LOG("i2c communication error");
		cal_result = PS_CAL_ERR_OTHER;
		goto cal_end_mdctrl_bak;
	}
	result = ps_als_i2c_write_byte_data(ps_als->client, REG_MODECONTROL, 0xF5);
	if(unlikely(result < 0)) {
		SENSOR_ERR_LOG("i2c communication error");
		cal_result = PS_CAL_ERR_OTHER;
		goto cal_end;
...
#endif
cal_end:
	ps_als_i2c_write_byte_data(ps_als->client, REG_MODECONTROL, (unsigned char)bak_modectrl);
...
void ps_get_threshold(uint16_t* th_detect, uint16_t* th_no_detect, uint16_t* distance_th)
{
	PS_ALS_DATA *ps_als = i2c_get_clientdata(client_rpr0521);
	SENSOR_D_LOG("start");
	*th_detect = (uint16_t)ps_als->init_data.psth_upper;
	*th_no_detect = (uint16_t)ps_als->init_data.psth_low;
...
static int ps_als_parse_dt(PS_ALS_DATA *ps_als)
{
	...
	int ret = 0 ;
	const char *desc = "rpr0521,irq-gpio";
	int chip_vendor = 0;
	...
	if(chip_vendor == 1){
		ps_als->irq_gpio = of_get_named_gpio(of_node, "rpr0521,irq-gpio", 0);
...
static void ps_als_regulator_low(PS_ALS_DATA *ps_als)
{
	...
	if (!ps_als->ps_als_vdd_is_ldo) {
		if(ps_als->ps_als_is_PMIC_supply_ctrl){
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 4, 0))
			err = regulator_set_load(ps_als->ps_als_vcc.v_reg, ps_als->ps_als_vcc.off_load_uA);
			if( err < 0 ) {
...
static void ps_als_regulator_normal(PS_ALS_DATA *ps_als)
{
	...
	if (!ps_als->ps_als_vdd_is_ldo) {
		if(ps_als->ps_als_is_PMIC_supply_ctrl){
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 4, 0))
			err = regulator_set_load(ps_als->ps_als_vcc.v_reg, ps_als->ps_als_vcc.on_load_uA);
			if( err < 0 ) {
				SENSOR_ERR_LOG("[ALS_PS]%s regulator_set_load fail. err=%d\n", __func__, err);
			}
#else
...
static int ps_als_i2c_read(const struct i2c_client *client, uint8_t reg, uint8_t *rbuf, int len)
{
	...
	while (retry--) {
		...
		if (ret == I2C_READ_MSG_NUM) {
			...
			return len;
		} else if((ret == -EBUSY) || (ret == -ETIMEDOUT) || (ret == -EINPROGRESS)) {
