...
static ssize_t acc_nrfilt_prm_show(struct device *dev,
    struct device_attribute *attr,
    char *buf);
static ssize_t acc_dynamic_range_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf,
    size_t count);
static ssize_t acc_dynamic_range_show(struct device *dev,
    struct device_attribute *attr,
    char *buf);
...
static DEVICE_ATTR(dynamic_range,
    S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP,
    acc_dynamic_range_show,
    acc_dynamic_range_store
);
    ...
    char *buf )
{
    ...
    raw[0] = acc_last_read_data.outX + offsets[0];
    raw[1] = acc_last_read_data.outY + offsets[1];
    raw[2] = acc_last_read_data.outZ + offsets[2];
    ...
    size_t count)
{
    ...
    int32_t ret;
    SENSOR_D_LOG("start");
    ret = kstrtou8(buf, 0, &drange_val);
    if(ret != 0){
        SENSOR_ERR_LOG("parse error.");
        return count;
    }
    sensor_com_mutex_lock();
    ret = sensor_set_dynamic_range(SENSOR_ACC, drange_val);
    sensor_com_mutex_unlock();
    if(ret != SNS_RC_OK){
        drange_val = 0xFF;
    }
    ...
    char *buf)
{
    ...
    SENSOR_D_LOG("start");
    sensor_com_mutex_lock();
    sensor_get_dynamic_range(SENSOR_ACC, &val);
    sensor_com_mutex_unlock();
    SENSOR_D_LOG("end");
...
static ssize_t acc_selfchk_exec(char* buf)
{
    ...
    sensor_enable( SENSOR_ACC, &acc_poll_info, false );
    sensor_set_selfchk_config(SENSOR_ACC, SELFCHK_OFF);
