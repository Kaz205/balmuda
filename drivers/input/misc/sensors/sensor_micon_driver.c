...
static struct sensor_ctrl_config_str sns_ctrl_config[SENSOR_MAX] = {
    ...
    [SENSOR_KC_MOTION_WAKEUPGESTURE]= { SENSOR_KC_MOTION_WAKEUPGESTURE,EN_TYPE_APP,       BT_TYPE_NONE,       EN_BIT_ACC,                         EN_BIT_NONE,            NULL },
    [SENSOR_KC_MOTION_FACEDOWN]     = { SENSOR_KC_MOTION_FACEDOWN,    EN_TYPE_APP,        BT_TYPE_NONE,       EN_BIT_ACC,                         EN_BIT_NONE,            NULL },
    [SENSOR_KC_MOTION_FLICK]        = { SENSOR_KC_MOTION_FLICK,       EN_TYPE_APP,        BT_TYPE_NONE,       EN_BIT_A_G,                         EN_BIT_NONE,            NULL },
    ...
    [SENSOR_ACC_UNCAL]              = { SENSOR_ACC_UNCAL,             EN_TYPE_NORMAL,     BT_TYPE_NORMAL,     EN_BIT_ACC|EN_BIT_ACC_UNCAL,        EN_BIT_ACC_UNCAL,       NULL }
    /* SENSOR_MAX */
};
...
static struct kthread_worker micon_poll_wq;
static struct kthread_work micon_work;
...
static int32_t sns_make_sensor_ctrl_info(struct sensor_ctrl_param_input_str* src, struct sensor_ctrl_param_output_str *dst)
{
    ...
    for (i = 0; i < ARRAY_SIZE(src->inputs); i++) {
        ...
        if (input->enable) {
            ...
            if (config->en_type == EN_TYPE_NORMAL) {
                ...
                if (config->en_bit_measure & (EN_BIT_ACC | EN_BIT_ACC_UNCAL))
                    period_acc = min(period_acc, input->sampling_period_ms);
                ...
                if (config->en_bit_logging & (EN_BIT_ACC | EN_BIT_ACC_UNCAL))
                    period_acc_logging = min(period_acc_logging, input->sampling_period_ms);
                ...
                if (config->en_bit_measure & EN_BIT_ACC) {
                    if (config->sns_type == SENSOR_DEVICE_ORIENTATION ||
                        ...
                        config->sns_type == SENSOR_KC_MOTION_WAKEUPGESTURE ||
                        config->sns_type == SENSOR_KC_MOTION_FACEDOWN ||
                        config->sns_type == SENSOR_KC_MOTION_FLICK)
                ...
                if (config->en_bit_measure & EN_BIT_GYRO){
                    if (config->sns_type == SENSOR_KC_MOTION_FLICK){
                        period_gyro = min(period_gyro, 30);
                    }
                }
                if (config->en_bit_measure & EN_BIT_PRESSURE) {
    ...
    if (!fusion_enable && app_enable && !android_sensor_enable) {
        if (test_bit(SENSOR_EXT_VH, sensor_enable_sns) ||
            ...
            test_bit(SENSOR_KC_MOTION_WAKEUPGESTURE, sensor_enable_sns) ||
            test_bit(SENSOR_KC_MOTION_FACEDOWN, sensor_enable_sns) ||
            test_bit(SENSOR_KC_MOTION_FLICK, sensor_enable_sns) ||
    ...
    period_acc_logging = sns_adjust_value_by_range_enable(period_acc_logging,
        IS_ACC_EN(logging_enable)||IS_ACC_UNCAL_EN(logging_enable),
        PERIOD_ACC_MAX, PERIOD_ACC_MIN, false);
    ...
    struct sensor_ctrl_param_output_str *new)
{
    ...
    int micon_period = get_fastest_polling_period();
    SENSOR_D_LOG("start");
    ...
    if (!old) {
        ...
        if (old->sensor.enable || new->sensor.enable) {
            ...
            } else if (micon_period != reculc_micon_polling(new->polling.enable_sns)) {
                change_type |= CHANGE_TYPE_POLLING;
            }
        }
    }
    ...
    struct sensor_ctrl_param_output_str *new)
{
    ...
    for (i = 0; i < SENSOR_MAX; i++) {
        ...
        if ((test_bit(i, changed_bit)) && (poll_info_p != NULL)) {
            if (test_bit(i, new->polling.enable_sns)) {
                ...
                if (sns_ctrl_config[i].sns_type == SENSOR_ACC_UNCAL) {
                    sns_micon_func.accuc_func.clear_filt_coff();
                }
    ...
    if(!bitmap_empty(new->polling.enable_sns, SENSOR_MAX)){
        ...
        kthread_cancel_work_sync(&micon_work);
    }
    SENSOR_N_LOG("end");
    return SNS_RC_OK;
}
...
int32_t sns_acc_set_dynamic_range(uint8_t input_drange)
{
    return sns_micon_func.acc_func.set_dynamic_range(input_drange);
}

int32_t sns_acc_get_dynamic_range(uint8_t *output_drange)
{
    return sns_micon_func.acc_func.get_dynamic_range(output_drange);
}
...
void sns_accuc_set_ts_nrfilt_baseinfo(int32_t base_val)
{
    sns_micon_func.accuc_func.set_ts_nrfilt_info(base_val);
}
...
int32_t sns_gyro_set_dynamic_range(uint8_t input_drange)
{
    return sns_micon_func.gyro_func.set_dynamic_range(input_drange);
}

int32_t sns_gyro_get_dynamic_range(uint8_t *output_drange)
{
    return sns_micon_func.gyro_func.get_dynamic_range(output_drange);
}

int32_t sns_accuncalib_read_data(struct acc_uncalib *arg_Acc)
{
    int32_t ret = SNS_RC_OK;
    SENSOR_N_LOG("start");
    sns_micon_func.get_sns_value(SENSOR_ACC_UNCAL, arg_Acc);
    SENSOR_N_LOG("end");
    return ret;
}
...
int32_t sns_kc_motion_wakeupgesture_start(bool arg_iStart)
{
    return sns_micon_func.kcmot_func.wakeupgesture_enable(arg_iStart);
}

int32_t sns_kc_motion_facedown_start(bool arg_iStart)
{
    return sns_micon_func.kcmot_func.facedown_enable(arg_iStart);
}

int32_t sns_get_kc_motion_facedown_data(struct kc_motion_facedown_data *arg_Facedown)
{
    return sns_micon_func.kcmot_func.facedown_get_data(arg_Facedown);
}

int32_t sns_kc_motion_flick_start(bool arg_iStart)
{
    return sns_micon_func.kcmot_func.flick_enable(arg_iStart);
}
...
int32_t sns_allapp_restart(void)
{
    ...
    status = sensor_get_status(SENSOR_KC_MOTION_WAKEUPGESTURE);
    if(status){
        SENSOR_ERR_LOG("Restart KCMotionWakeupGesture");
        ret = sns_kc_motion_wakeupgesture_start(true);
    }
    status = 0;

    status = sensor_get_status(SENSOR_KC_MOTION_FACEDOWN);
    if(status){
        SENSOR_ERR_LOG("Restart KCMotionFaceDown");
        ret = sns_kc_motion_facedown_start(true);
    }
    status = 0;

    status = sensor_get_status(SENSOR_KC_MOTION_FLICK);
    if(status){
        SENSOR_ERR_LOG("Restart KCMotionFlick");
        ret = sns_kc_motion_flick_start(true);
    }
    status = 0;
#endif
...
static void micon_poll_work_func(struct kthread_work *work)
{
    SENSOR_D_LOG("start");
...
enum hrtimer_restart micon_poll(struct hrtimer *micon_timer)
{
    ...
    SENSOR_D_LOG("start");
    kthread_queue_work(&micon_poll_wq, &micon_work);
...
static void sns_micon_init(void)
{
    ...
    kthread_init_work(&micon_work, micon_poll_work_func);
    kthread_init_worker(&micon_poll_wq);
    task = kthread_run(kthread_worker_fn, &micon_poll_wq, "micon_poll");
    if (IS_ERR(task)) {
        kthread_stop(task);
        task = NULL;
        SENSOR_D_LOG("micon_poll kthread cannot run!");
        return;
    }
