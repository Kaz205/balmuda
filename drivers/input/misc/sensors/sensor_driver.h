...
printk(KERN_ERR "[SENSOR][%s][F](%d): " msg "\n", __func__, __LINE__, ## __VA_ARGS__)
  ...
  #if defined (CONFIG_INPUT_SENSOR_YAS_MAG)
...
enum sensor_e_type{
    ...
    SENSOR_KC_MOTION_WAKEUPGESTURE,
    SENSOR_KC_MOTION_FACEDOWN,
    SENSOR_KC_MOTION_FLICK,
...
struct kc_motion_wakeupgesture_data {
    uint32_t status;
};

struct kc_motion_facedown_data {
    uint32_t status;
};

struct kc_motion_flick_data {
    uint32_t status;
};
...
union sensor_read_data_u {
    ...
    struct acc_uncalib acc_uncal_data;
...
union sensor_motion_read_data_u {
    ...
    struct kc_motion_wakeupgesture_data wakeupgesture_data;
    struct kc_motion_facedown_data facedown_data;
    struct kc_motion_flick_data flick_data;
};
