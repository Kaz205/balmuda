    ...
    bool enable )
{
    ...
    if( true == enable ){
        if( SENSOR_ON == sensor_get_status_local(type) &&
            SENSOR_SGNFCNT_MTN != type && SENSOR_KC_MOTION_WAKEUPGESTURE != type && SENSOR_KC_MOTION_FLICK != type) {
...
static int32_t sensor_application_enable(enum sensor_e_type type,bool enable)
{
    ...
    case SENSOR_KC_MOTION_WAKEUPGESTURE:
        ret = sns_kc_motion_walk_start_start( enable );
        ret = sns_kc_motion_wakeupgesture_start( enable );
        break;
    case SENSOR_KC_MOTION_FACEDOWN:
        ret = sns_kc_motion_facedown_start( enable );
        break;
    case SENSOR_KC_MOTION_FLICK:
        ret = sns_kc_motion_flick_start( enable );
        break;
    case SENSOR_UNDERWATER_DETECT:
...
static void sensor_inform_ts_noize_reduction_info(const char *wqname, int32_t delay_ms)
{
    ...
    else if(!strcmp(wqname, "acc_uncal_poll_wq")){
        SENSOR_D_LOG("try to set acc_uc ts_base_val.");
        sns_accuc_set_ts_nrfilt_baseinfo(delay_ms);
    }
    ...
    union sensor_read_data_u* read_data )
{
    ...
    case SENSOR_GRV:
        ...
        break;
    case SENSOR_ACC_UNCAL:
        ret = sns_accuncalib_read_data( &(read_data->acc_uncal_data) );
        break;
    case SENSOR_GYRO_UNCAL:
...
void sensor_enable_with_suspend(
    enum sensor_e_type type,
    struct sensor_poll_info_str* poll_info,
    bool enable
)
{
    ...
    status = sensor_drv_get_status_local();
    if (status != SENSOR_RESUME && status != SENSOR_SUSPEND){
        SENSOR_ERR_LOG("bad status-->sensor_drv_status[%d]",
                       (int)status );
        mutex_unlock(&sensor_com_mutex);
        return;
...
void sensor_notify( enum sensor_e_type type, uint32_t data)
{
    ...
    case SENSOR_EXT_PEDO:
#ifdef CONFIG_INPUT_SENSOR_EXT
        sensor_ext_pedo_interrupt(data);
#endif
        break;
    case SENSOR_EXT_VEHI:
#ifdef CONFIG_INPUT_SENSOR_EXT
    ...
    case SENSOR_KC_MOTION_WAKEUPGESTURE:
#ifdef CONFIG_INPUT_KC_MOTION_SENSOR
        kc_motion_sensor_wakeupgesture_interrupt();
#endif
        break;
    case SENSOR_KC_MOTION_FACEDOWN:
#ifdef CONFIG_INPUT_KC_MOTION_SENSOR
        kc_motion_sensor_facedown_interrupt();
#endif
        break;
    case SENSOR_KC_MOTION_FLICK:
#ifdef CONFIG_INPUT_KC_MOTION_SENSOR
        kc_motion_sensor_flick_interrupt();
#endif
        break;
...
int32_t sensor_set_dynamic_range(enum sensor_e_type type, uint8_t drange_val)
{
    int32_t ret = SNS_RC_OK;
    SENSOR_D_LOG("start");
    switch (type){
        case SENSOR_ACC:
            ret = sns_acc_set_dynamic_range(drange_val);
            break;
        case SENSOR_GYRO:
            ret = sns_gyro_set_dynamic_range(drange_val);
            break;
        default:
            SENSOR_ERR_LOG("Invalid type[%d]", type);
            break;
...
int32_t sensor_get_dynamic_range(enum sensor_e_type type, uint8_t *drange_val)
{
    int32_t ret = SNS_RC_OK;
    SENSOR_D_LOG("start");
    switch (type){
        case SENSOR_ACC:
            ret = sns_acc_get_dynamic_range(drange_val);
            break;
        case SENSOR_GYRO:
            ret = sns_gyro_get_dynamic_range(drange_val);
            break;
        default:
            SENSOR_ERR_LOG("Invalid type[%d]", type);
            break;
...
static void sensor_suspend_ctl( void )
{
    ...
    if(( SENSOR_ON == sensor_get_status_local(SENSOR_SGNFCNT_MTN) )||
       ...
       ( SENSOR_ON == sensor_get_status_local(SENSOR_KC_MOTION_WAKEUPGESTURE) )||
       ( SENSOR_ON == sensor_get_status_local(SENSOR_KC_MOTION_FACEDOWN) )||
       ( SENSOR_ON == sensor_get_status_local(SENSOR_KC_MOTION_FLICK) )||
...
static void sensor_set_not_off_suspend_type_bitmap(void)
{
    ...
    set_bit(SENSOR_KC_MOTION_WAKEUPGESTURE, sensor_not_off_suspend_type);
    set_bit(SENSOR_KC_MOTION_FACEDOWN, sensor_not_off_suspend_type);
    set_bit(SENSOR_KC_MOTION_FLICK, sensor_not_off_suspend_type);
    SENSOR_N_LOG("end");
    return;
}
