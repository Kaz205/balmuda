...
#define EN_BIT_ACC_UNCAL            (0x02000000)
...
#define IS_ACC_UNCAL_EN(enable_bits)        (((enable_bits) & EN_BIT_ACC_UNCAL) != 0)
...
typedef enum {
    ...
    SHID_META_DATA,
    SHID_ACCELEROMETER_UNCALIBRATED,
    SHID_MAX,
} SENSOR_HUB_EVENT_ID;
...
/* Accelerometer Dynamic Range definition */
#define ACC_DYNAMIC_RANGE_16G       (0x00) /* +-16g */
#define ACC_DYNAMIC_RANGE_8G        (0x01) /* +-8g */
#define ACC_DYNAMIC_RANGE_4G        (0x02) /* +-4g */
#define ACC_DYNAMIC_RANGE_2G        (0x03) /* +-2g */
#define ACC_DYNAMIC_RANGE_INVALID   (0x04) /* INVALID */

/* For converting accelerometer unit */
#if defined (CONFIG_USE_SENSOR_LSM6DS3)
    const static int32_t acc_unitconv_factor[] = {16, 8, 4, 2, 4};
    #define CULC_ACC_VAL_DIGIT_2_MG(digit, factor)    ((digit * factor * 305) / 10000)
#elif defined (CONFIG_USE_SENSOR_LIS2DH12)
    const static int32_t acc_unitconv_factor[] = {48, 16, 8, 4, 8};
    #define CULC_ACC_VAL_DIGIT_2_MG(digit, factor)    ((digit * factor) / 64)
#else
    const static int32_t acc_unitconv_factor[] = {1, 1, 1, 1};
    #define CULC_ACC_VAL_DIGIT_2_MG(digit, factor)    ((digit * factor) / 64)
#endif

/* Gyroscope Dynamic Range definition */
#define GYRO_DYNAMIC_RANGE_245DPS   (0x00) /* +-245dps */
#define GYRO_DYNAMIC_RANGE_500DPS   (0x01) /* +-500dps */
#define GYRO_DYNAMIC_RANGE_1000DPS  (0x02) /* +-1000dps */
#define GYRO_DYNAMIC_RANGE_2000DPS  (0x03) /* +-2000dps */
#define GYRO_DYNAMIC_RANGE_125DPS   (0x04) /* +-125dps */
#define GYRO_DYNAMIC_RANGE_INVALID  (0x05) /* INVALID */

/* For converting gyroscope unit */
#if defined (CONFIG_USE_SENSOR_LSM6DS3)
    const static int32_t gyro_unitconv_factor[] = {8, 4, 2, 1, 16, 1};
#else
    const static int32_t gyro_unitconv_factor[] = {1, 1, 1, 1, 1};
#endif
#define CULC_GYRO_VAL_DIGIT_2_0P1DPS(digit) (digit * 70 / 100)
#define CULC_GYROUC_VAL_DIGIT_2_0P1DPS(digit) (digit * 70 / 100)

/* For converting magnetometer unit */
#define CULC_MAG_VAL_DIGIT_2_0P1UT(digit)   ((digit * 15) / 10)

/* For converting magnetometer unit */
#if defined (CONFIG_USE_SENSOR_LPS22HH)
    #define CULC_PRESS_VAL_DIGIT_2_PA(digit)    ((digit * 100) / 4096)
#elif defined (CONFIG_USE_SENSOR_BM1383AGLV)
    #define CULC_PRESS_VAL_DIGIT_2_PA(digit)    ((digit * 100) / 2048)
#endif
...
struct acceleration_func {
    ...
    int32_t (*set_dynamic_range)(uint8_t dynamic_range);
    int32_t (*get_dynamic_range)(uint8_t *dynamic_range);
};
...
struct kc_motion_det_func {
    ...
    int32_t (*wakeupgesture_enable)(bool enable);
    int32_t (*facedown_enable)(bool enable);
    int32_t (*facedown_get_data)(struct kc_motion_facedown_data *arg_Facedown);
    int32_t (*flick_enable)(bool enable);
};
...
int32_t sns_kc_motion_wakeupgesture_activate(bool arg_iEnable);
int32_t sns_kc_motion_wakeupgesture_start(bool arg_iStart);
int32_t sns_kc_motion_facedown_activate(bool arg_iEnable);
...
int32_t sns_kc_motion_flick_activate(bool arg_iEnable);
int32_t sns_kc_motion_flick_start(bool arg_iStart);
int32_t sns_get_kc_motion_flick_data(struct kc_motion_flick_data *arg_Flick);
