...
#define EXT_FUNC_MICON_DEAD     0x20
...
static int32_t prv_timestamp = 0;
...
static ssize_t sensor_ext_sound_vib_prevention_store(struct device *dev,
    ...
    const char *buf,
    size_t count);
static ssize_t sensor_ext_interrupt_kind_show(struct device *dev,
    struct device_attribute *attr,
    char *buf );
...
static DEVICE_ATTR(dailys_interrupt_kind,
    S_IRUSR|S_IRGRP,
    sensor_ext_interrupt_kind_show,
    NULL
);
static struct attribute *sensor_ext_attributes[] = {
    ...
    &dev_attr_dailys_interrupt_kind.attr,
    NULL
};
	...
	char *buf)
{
	...
	ret = sprintf( buf, "%x\n",
				   atomic_read(&sns_ext_pedom_interrupt_kind) );
	SENSOR_N_LOG("interrupt kind = %x",atomic_read(&sns_ext_pedom_interrupt_kind));
...
void sensor_ext_pedo_interrupt( uint32_t data )
{
    ...
    if ( SENSOR_ON == sensor_get_status(SENSOR_EXT_PEDO) ) {
		...
		if (data == 99) {
			atomic_set(&sns_ext_pedom_interrupt_kind,EXT_FUNC_MICON_DEAD);
		} else {
...
#define DAILYS_EVENT_WALK_START     (0x00)
...
void sensor_ext_input_report(int64_t clear_timestamp, void *event, uint32_t len)
{
    ...
    int32_t evtime_prv_to_cur_timestamp = 0;
    int32_t step_cnt = 0;
    int report_cnt = 0;
    ...
    while (len) {
        ...
        if (header < DAILYS_EVENT_HEADER_MAX && info->type != 0) {
            if (info->type == 1) {
                ...
                evtime_prv_to_cur_timestamp = ev_ts - prv_timestamp;
                /* For divide by zero */
                if (evtime_prv_to_cur_timestamp == 0) {
                    evtime_prv_to_cur_timestamp = 1;
                }
                /* For timestamp overflow */
                if (evtime_prv_to_cur_timestamp < 0) {
                    evtime_prv_to_cur_timestamp += (0xFFFF + 1);
                }
                if (buf[0] == DAILYS_EVENT_WALK_START) {
                    prv_timestamp =  ev_ts;
                }
                if (buf[0] == DAILYS_EVENT_SWITCH_WALKRUN) {
                    step_cnt = ((int32_t)buf[9] & 0x000000FF) | (((int32_t)buf[10] & 0x000000FF) << 8);
                    if ((step_cnt >= 0xFFF0) && (step_cnt / (evtime_prv_to_cur_timestamp * 3)) >= 10) {
                        SENSOR_ERR_LOG("invalid step_cnt = %d", step_cnt);
                        buf[9]  = (uint8_t)(((evtime_prv_to_cur_timestamp * 3) / 2) & 0x000000FF);
                        buf[10] = (uint8_t)((((evtime_prv_to_cur_timestamp * 3) / 2) & 0x0000FF00) >> 8);
                        prv_timestamp = ev_ts;
                     }
                }
                if (buf[0] ==DAILYS_EVENT_WALK_END) {
                    step_cnt = ((int32_t)buf[6] & 0x000000FF) | (((int32_t)buf[7] & 0x000000FF) << 8);
                    if ((step_cnt >= 0xFFF0) && (step_cnt / (evtime_prv_to_cur_timestamp * 3)) >= 10) {
                        SENSOR_ERR_LOG("invalid step_cnt = %d", step_cnt);
                        buf[6] = (uint8_t)(((evtime_prv_to_cur_timestamp * 3) / 2) & 0x000000FF);
                        buf[7] = (uint8_t)((((evtime_prv_to_cur_timestamp * 3) / 2) & 0x0000FF00) >> 8);
                        prv_timestamp = ev_ts;
                    }
                }
