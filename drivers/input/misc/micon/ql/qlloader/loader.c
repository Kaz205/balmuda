...
static bool is_initial_fw_download = true;
uint32_t fw_image_size=0;
static uint8_t *fw_image_buf=NULL;
#ifdef FABRIC_LOADER
uint32_t fabric_image_size=0;
uint32_t fabricprog_image_size=0;
static uint8_t *fabric_image_buf=NULL;
static uint8_t *fabricprog_image_buf=NULL;
#endif
#ifdef FFE_LOADER
uint32_t ffe_image_size=0;
static uint8_t *ffe_image_buf=NULL;
#endif
...
static void set_known_bin_info(SLAVE_DEV_FW_LOAD_T *slave_dev_fw_load_info)
{
	LDR_DBG("Start\n");
	...
	slave_dev_fw_load_info->fab_prg_fw_addr = fabricprog_image_buf;
	slave_dev_fw_load_info->fab_prg_fw_size = fabricprog_image_size;
...
static int load_fw_info(SLAVE_DEV_FW_LOAD_T *slave_dev_fw_load_info)
{
	LDR_DBG(" Load FW Bin info start.");
	if(!fw_image_buf) {
	...
	if(!fabricprog_image_buf) {
		if(load_fw_to_arr(&fabricprog_image_buf ,&fabricprog_image_size,FABRIC_PROG_FW_FILENAME)!=QL_STATUS_OK) {
...
#endif
	LDR_DBG(" load FW Bin info normal end.");
	return QL_STATUS_OK;
}
...
static ssize_t s3_fw_download(struct device *dev, struct device_attribute *attr,const char *buf, size_t count)
{
	...
	if(is_initial_fw_download){
		memset(&slave_dev_fw_load_info,0,sizeof(SLAVE_DEV_FW_LOAD_T));
		//read fw into a buffer
		if(load_fw_info(&slave_dev_fw_load_info) != QL_STATUS_OK){
			QLSPI_ERR("load firmware bin from user failed.");
			return -1;
		}
		is_initial_fw_download = false;
	} else {
		set_known_bin_info(&slave_dev_fw_load_info);
		QLSPI_DBG("firmware bin info has already known.");
	}
