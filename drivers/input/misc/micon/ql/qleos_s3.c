...
static bool is_qleoss3_init_running = true;
static atomic_t g_sns_flush_type;
static atomic_t g_is_flush_exec;
static atomic_t current_hostcmd = ATOMIC_INIT(0x00000000);
...
static void set_current_hostcmd(const int32_t command);
static int32_t get_current_hostcmd(void);
...
#define FFE_EXTREGS_STATUS_ADDR  0x4004A110
#define FFE_EXTREGS_STATUS_FFE0_BUSY (1 << 2)
...
#ifdef USE_QA_LOG
		qlspi_linux_notice("DEAD:hw_reboot is occured High frequency. kill eoss3.");
#endif
...
#ifdef USE_QA_LOG
	qlspi_linux_notice("RECOVERING:Recoverying during recovery has occured. restart EOSS3 recovery.");
#endif
	hw_reboot_exec();
	EXITED();
}
...
#ifdef USE_QA_LOG
		qlspi_linux_notice("RECOVERING:Request hw_reboot. client");
#endif
		hw_reboot_exec();
	}
	EXITED();
...
static bool chk_ignore_hostcmdError(int32_t hostcmd)
{
	...
	switch(hostcmd){
		...
		case PRESS_INIT_CMD:
		case TEMP_INIT_CMD:
		case PRESS2_INIT_CMD:
		case SENS_PROC_ENABLE_CMD:
		case SENS_PROC_PERIOD_CMD:
		case SENS_BATCH_ENABLE_CMD:
		case SENS_BATCH_PERIOD_CMD:
		case FFE_TIMER_ENABLE_CMD:
			QLEOS_ERR("command error[0x%08x]. Ignore micon recovery sequence.", hostcmd);
			is_ignore = true;
			break;
		default:
			break;
...
QL_Status qleos_s3_isr(void * data)
{
	int length = 8;
	int ret = 0;
	int32_t cur_hostcmd = 0x00000000;
	...
	uint32_t reg_ffe_stat = 0;
	enum EOS_reboot_mode reboot_mode = POWER_OFFON_REBOOT;
	...
	if(atomic_read(&pending_hc_cmplt) && !intr_isr_after_hc_done){
		...
		if( (ret == -1) || (ret == SPI_STATUS_ERR_AHB) || (ret == SPI_STATUS_ERR_DMA) ){
			...
			mutex_unlock(&hc_irq_mutual);
#ifdef USE_QA_LOG
		        qlspi_linux_notice("RECOVERING:EIF error. reboot hw.");
#endif
			goto hw_reboot;
		}
	...
	if (result & other_factor) {
		...
		if (result & HOST_CMD_ERR) {
			cur_hostcmd = get_current_hostcmd();
			QLEOS_ERR("Host Command Error Occured[hc:0x%08x]\n", cur_hostcmd);
			intr_irq_flag &= ~(result & HOST_CMD_ERR);
			//error process about Result bit60-62 are nonzero.
			if(!chk_ignore_hostcmdError(cur_hostcmd)){
				mutex_unlock(&data_mutex);
				mutex_unlock(&eos3_irq_mutex);
#ifdef USE_QA_LOG
		                qlspi_linux_notice("RECOVERING:Host Command Error Occured");
#endif
				goto hw_reboot;
			}
		}
		if (result & INTREQ_ERROR){
			...
			if (QLSPI_Read_S3_Mem(FFE_EXTREGS_STATUS_ADDR, (uint8_t*)&reg_ffe_stat, 4) != QL_STATUS_OK) {
				QLEOS_ERR("QLSPI_Read_S3_Mem error.\n");
			} else {
				QLSPI_ERR("FFE STATUS: 0x%x \n", reg_ffe_stat);
				if (reg_ffe_stat & FFE_EXTREGS_STATUS_FFE0_BUSY) {
					reboot_mode = HW_RESET_REBOOT;
				}
			}
			mutex_unlock(&data_mutex);
			mutex_unlock(&eos3_irq_mutex);
			//get_error_factor();
#ifdef USE_QA_LOG
	                qlspi_linux_notice("RECOVERING:INTREQ Error Occured");
#endif
			goto hw_reboot;
		}
		if (result & WDT_EXPIRED){
			...
			mutex_unlock(&eos3_irq_mutex);
#ifdef USE_QA_LOG
	                qlspi_linux_notice("RECOVERING:WDT Expired Occured");
#endif
			goto hw_reboot;
		}
		if (result & INTREQ_BOOT_DONE) {
			...
			if (current_eos_status == FW_DOWNLOADING) {
				...
				mutex_unlock(&eos3_irq_mutex);
#ifdef USE_QA_LOG
				qlspi_linux_notice("RECOVERING:Unexpected BootDone error");
#endif
				goto hw_reboot;
			}
		}
	...
	return 0;
hw_reboot:
	ql_spi_set_eoss3_reboot_mode(reboot_mode);
	hw_reboot_exec();
	EXITED();
	return -1;
}
...
static void set_current_hostcmd(const int32_t command)
{
	atomic_set(&current_hostcmd, command);
}

static int32_t get_current_hostcmd(void)
{
	return (int32_t)atomic_read(&current_hostcmd);
}
...
int qleoss3_hostcmd(struct hc_info *ihc)
{
	...
	if (ret == QL_STATUS_OK) {
		...
		mutex_unlock(&hc_irq_mutual);
		set_current_hostcmd(ihc->command);
		ret = qleoss3_waitcmd();
		if (ret != QL_STATUS_OK) {
			if (ret == -QL_STATUS_ERROR) {
				...
				mutex_unlock(&hc_exec);
#ifdef USE_QA_LOG
				qlspi_linux_notice("RECOVERING:Host command timeout");
#endif
				hw_reboot_exec();
				return -QL_STATUS_ERROR;
			}
