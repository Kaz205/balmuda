...
static struct control_data sens_sleep_clk2 = { "sens_sleep_clk2" };
static struct control_data* data_list[] = { &vmicon_11, &vmicon_18, &sens_sleep_clk, &vsensor_18, &sens_sleep_clk2};
			...
			if (data->ctrl == CTRL_PINCTRL) {
				if (data == &vmicon_11) {
					...
					data->pinctrl_suspend = QLPIN_VSENSOR_18_SUSPEND;
                                ...
                                else if (data == &sens_sleep_clk2) {
                                        data->pinctrl_active = QLPIN_SLEEP_CLK2_ACTIVE;
                                        data->pinctrl_suspend = QLPIN_SLEEP_CLK2_SUSPEND;
                                }
			}
		...
		if (!qlpower_discharged) {
			...
			if (ret) {
				...
				goto err;
                        ...
                        ret = qlpower_control_enable(&sens_sleep_clk2, 1);
                        if (ret) {
                                LOGE("qpnp_pin_config sens_sleep_clk2 enable fail. ret=%d", ret);
                                goto err;
                        }
...
#endif /* CONFIG_QL_MICON_USE_EXTERNAL_SLEEPCLK */
                        ret = qlpower_control_enable(&sens_sleep_clk2, 0);
                        if (ret) {
                                LOGE("qpnp_pin_config sens_sleep_clk2 disable fail. ret=%d", ret);
                                goto err;
                        }
