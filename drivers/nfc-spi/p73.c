...
struct pinctrl_state *spi_ese_active, *spi_ese_suspend;
struct pinctrl_state *spi_ese_init,   *spi_ese_deinit;
...
void spi_ese_pinctrl(int status)
{
    ...
    if(pinctrl_spi == NULL)
    {
        P61_ERR_MSG("%s : pinctrl_spi is NULL!!\n", __func__);
        return;
    }

    if(spi_ese_active == NULL)
    {
        P61_ERR_MSG("%s : spi_ese_active is NULL!!\n", __func__);
        return;
    }

    if(spi_ese_suspend == NULL)
    {
        P61_ERR_MSG("%s : spi_ese_suspend is NULL!!\n", __func__);
        return;
    }

    if (status)
    {
        pinctrl_select_state(pinctrl_spi, spi_ese_active);
    }
    else
    {
        pinctrl_select_state(pinctrl_spi, spi_ese_suspend);
    }
...
// In init case, This function is called after IO_PWR_ENABLE is High.
// In deinit case, This function is called after IO_PWR_ENABE is Low.
int spi_ese_pinctrl_init(int status)
{
    int ret = 0;
    ...
    if(spi_ese_init == NULL)
    {
        P61_ERR_MSG("%s : spi_ese_init is NULL!!\n", __func__);
        return -1;
    }

    if(spi_ese_deinit == NULL)
    {
        P61_ERR_MSG("%s : spi_ese_deinit is NULL!!\n", __func__);
        return -1;
    }

    if (status)
    {
        ret = pinctrl_select_state(pinctrl_spi, spi_ese_init);
    }
    else
    {
        ret = pinctrl_select_state(pinctrl_spi, spi_ese_deinit);
    }
...
//    unsigned char tx_buffer[MAX_BUFFER_SIZE];
    ...
    tx_buffer=kzalloc(MAX_BUFFER_SIZE ,GFP_KERNEL);
    if (tx_buffer == NULL)
    {
...
//    if (copy_from_user(&tx_buffer[0], &buf[0], count))
    if (copy_from_user(tx_buffer, &buf[0], count))
    {
...
//    unsigned char rx_buffer[MAX_BUFFER_SIZE];
    ...
    rx_buffer=kzalloc(MAX_BUFFER_SIZE ,GFP_KERNEL);
    if (rx_buffer == NULL)
    {
        P61_ERR_MSG("failed to allocate memory for module data\n");
        ret = -ENOMEM;
...
//  p61_dev->rst_gpio  = platform_data->rst_gpio;
    ...
    if (IS_ERR(pinctrl_spi)) {
        ...
        pr_info("%s :  Cannot find pinctrl_spi! ret = %d\n", __func__, ret);
        goto err_exit0;
    }
    spi_ese_active = pinctrl_lookup_state(pinctrl_spi, "spi_ese_active");
    if (IS_ERR(spi_ese_active)) {
        ret = PTR_ERR(spi_ese_active);
        pr_info("%s :  Cannot find spi_ese_active! ret = %d\n", __func__, ret);
        goto err_exit0;
    }
    spi_ese_suspend = pinctrl_lookup_state(pinctrl_spi, "spi_ese_suspend");
    if (IS_ERR(spi_ese_suspend)) {
        ret = PTR_ERR(spi_ese_suspend);
        pr_info("%s :  Cannot find spi_ese_suspend! ret = %d\n", __func__, ret);
        goto err_exit0;
    }

    spi_ese_init = pinctrl_lookup_state(pinctrl_spi, "spi_ese_init");
    if (IS_ERR(spi_ese_init)) {
        ret = PTR_ERR(spi_ese_init);
        pr_info("%s :  Cannot find spi_ese_init! ret = %d\n", __func__, ret);
        goto err_exit0;
    }
    spi_ese_deinit = pinctrl_lookup_state(pinctrl_spi, "spi_ese_deinit");
    if (IS_ERR(spi_ese_deinit)) {
        ret = PTR_ERR(spi_ese_deinit);
        pr_info("%s :  Cannot find spi_ese_deinit! ret = %d\n", __func__, ret);
        goto err_exit0;
    }
