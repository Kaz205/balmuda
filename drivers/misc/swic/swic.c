...
#define SWIC_CCLOGIC_REG_INVALID_STATE  0xFF
...
static u8 dm_info_ccic_reg01_state = SWIC_CCLOGIC_REG_INVALID_STATE; /* not support */
static u8 dm_info_ccic_reg02_state = SWIC_CCLOGIC_REG_INVALID_STATE; /* not support */
static u8 dm_info_ccic_reg03_state = SWIC_CCLOGIC_REG_INVALID_STATE; /* not support */
static u8 dm_info_ccic_reg04_state = SWIC_CCLOGIC_REG_INVALID_STATE; /* not support */
...
static struct i2c_client *wake_lock;
...
static void swic_notify_work(struct work_struct *work);
static void swic_notify_from_cclogic(swic_state_enum en_state);
...
void swic_notify(swic_state_enum state)
{
	SWIC_DBG_PRINT("%s:### state=[0x%02x]\n", __func__, state);
	swic_info->notify_state = state;
	schedule_work(&swic_info->notify_work);
}

static void swic_notify_work(struct work_struct *work)
{
	struct pi3a6386_swic *sw = container_of(work, struct pi3a6386_swic, notify_work);

	SWIC_DBG_PRINT("%s:### work queue start\n", __func__);
	/* wake_lock */
	pm_stay_awake(&wake_lock->dev);

	swic_notify_from_cclogic(sw->notify_state);

	/* wake_unlock */
	pm_relax(&wake_lock->dev);

	SWIC_DBG_PRINT("%s:### work queue end\n", __func__);
}
...
static int32_t swic_earphone_in_proc_reg_w_all_sw_on(void)
{
	...
	else
	{
		...
		SWIC_DBG_PRINT("%s:###[RegW] All Switches ON(Success!) err_cnt=[%d/%d]\n",
								__func__, swic_info->earphone_in_proc_info.all_sw_on_reg_w.err_cnt, earphone_in_proc_tbl[EN_EARPHONE_IN_PROC_REG_W_ALL_SW_ON].retry_num);
		/* HS_DET_EN (SDM_GPIO33) =High */
		gpio_set_value(g_swic_hs_det_gpio, 1);
		SWIC_DBG_PRINT("%s:###[GPIO] HS_DET_EN(SDM_GPIO33) =High\n", __func__);
...
static int32_t swic_remove_earphone_proc(void)
{
	...
	/* HS_DET_EN(SDM_GPIO33) = Low */
	gpio_set_value(g_swic_hs_det_gpio, 0);
	SWIC_DBG_PRINT("%s:### [GPIO] HS_DET_EN(SDM_GPIO33) =Low\n", __func__);
...
static int32_t swic_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* wake_lock_init */
	wake_lock = client;
	device_init_wakeup(&wake_lock->dev, true);
	INIT_WORK(&swic_info->notify_work, swic_notify_work);
...
static void swic_shutdown(struct i2c_client *client)
{
	...
	i2c_set_clientdata(client, NULL);
	cancel_work_sync(&swic_info->notify_work);
    /* wake_lock_destroy */
	device_init_wakeup(&wake_lock->dev, false);
