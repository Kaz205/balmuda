...
static char   kerrsmem_log[KERR_SMEM_LOG_SIZE - 1];
              ...
              loff_t *ppos)
{
    ...
    memcpy_fromio(kerrsmem_log, kerr_devp->smem->log, sizeof(kerrsmem_log));

    if (copy_to_user(buf, kerrsmem_log, ret)) {
        printk(KERN_ERR "><Copy log to user buffer failed!\n");
        ret = -1;
    }
    /* Clear log buffer and state flag */
    memset_io(kerr_devp->smem->log, 0x00, count);
