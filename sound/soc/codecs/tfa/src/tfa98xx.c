...
static struct snd_soc_component *kaudio_component = NULL;
#endif

static uint16_t tfa98xx_vol_value = 0;  /* EB1065A AUD_18_0035 */
...
#if 1
int tfa98xx_get_speaker_swap(struct snd_kcontrol *kcontrol)
{
	if (!g_tfa98xx)
		return -EINVAL;

	/* return 0 - [Left:Lch Right:Rch] */
	/* return 1 - [Left:Rch Right:Lch] */
	return TFA_GET_BF(g_tfa98xx->handle, TDMLS);
}
EXPORT_SYMBOL(tfa98xx_get_speaker_swap); /* GB1003A AUD_18_0003 */

void tfa98xx_set_speaker_swap(struct snd_kcontrol *kcontrol, uint16_t value)
{
	if (!g_tfa98xx)
		return;
	...
	case 0:
		/* Left:Lch Right:Rch */
		pr_info("set Left:Lch Right:Rch\n");
		TFA_SET_BF(g_tfa98xx->handle, TDMRS, 1);
		TFA_SET_BF(g_tfa98xx->handle, TDMLS, 0);
		break;
	case 1:
		/* Left:Rch Right:Lch */
		pr_info("set Left:Rch Right:Lch\n");
		TFA_SET_BF(g_tfa98xx->handle, TDMRS, 0);
		TFA_SET_BF(g_tfa98xx->handle, TDMLS, 1);
		break;
	default:
		pr_err("tfa98xx_set_speaker_swap() unknown param\n");
		break;
	}
}
EXPORT_SYMBOL(tfa98xx_set_speaker_swap); /* GB1003A AUD_18_0003 */

void tfa_set_calibration_flag(struct snd_soc_component *component)
{
	struct tfa98xx *tfa98xx = snd_soc_component_get_drvdata(component);
	tfaSetAcsAndCalibrationFlag(tfa98xx->handle);
}
EXPORT_SYMBOL(tfa_set_calibration_flag);

int tfa98xx_mtpex_get(struct snd_soc_component *component, unsigned int *val)
{
	struct tfa98xx *tfa98xx = snd_soc_component_get_drvdata(component);
	enum Tfa98xx_Error err, status;
	unsigned short value;
...
EXPORT_SYMBOL(tfa98xx_mtpex_get);

struct snd_soc_component* tfa98xx_component_get(void)
{
	return kaudio_component;
}
EXPORT_SYMBOL(tfa98xx_component_get);

int tfa98xx_get_audio_control_vol(struct snd_kcontrol *kcontrol)
{
	int value_vol, value_volsec;

	if (!g_tfa98xx)
		return -EINVAL;

	value_vol = TFA_GET_BF(g_tfa98xx->handle, VOL);
	value_volsec = TFA_GET_BF(g_tfa98xx->handle, VOLSEC);

	if (value_vol != value_volsec)
		pr_err("VOL:[%d] VOLSEC:[%d]\n", value_vol, value_volsec);

	return value_vol;
}
EXPORT_SYMBOL(tfa98xx_get_audio_control_vol);

void tfa98xx_set_audio_control_vol(struct snd_kcontrol *kcontrol, uint16_t value)
{
	if (!g_tfa98xx)
		return ;

	pr_info("set VOL/VOLSEC value:%d\n", value);
	TFA_SET_BF(g_tfa98xx->handle, VOL,    value);
	TFA_SET_BF(g_tfa98xx->handle, VOLSEC, value);
	tfa98xx_vol_value = value;      /* EB1065A AUD_18_0035 */

}
EXPORT_SYMBOL(tfa98xx_set_audio_control_vol);
#endif
...
static void tfa98xx_add_widgets(struct tfa98xx *tfa98xx)
{
	...
	struct snd_soc_dapm_context *dapm = &tfa98xx->component->dapm;
#else
							...
							unsigned int mask)
{
	...
	if (reg & errs & ~mask) {
		...
		dev_info(tfa98xx->component->dev, "status_reg events: 0x%x\n", reg);
	}
...
static void tfa98xx_tapdet_work(struct work_struct *work)
{
	...
	/* Check for SPKS bit*/
	val =  snd_soc_component_read32(tfa98xx->component, TFA98XX_STATUSREG);
...
static void tfa98xx_dsp_init(struct tfa98xx *tfa98xx)
{
	...
	if (tfa98xx->init_count < TF98XX_MAX_DSP_START_TRY_COUNT) {
		...
		if (ret != Tfa98xx_Error_Ok) {
			...
			/* EB1065A Start AUD_18_0035 */
			if(tfa98xx_vol_value != 0){
				pr_info("resetting VOL/VOLSEC value:%d\n", tfa98xx_vol_value);
				TFA_SET_BF(g_tfa98xx->handle, VOL,    tfa98xx_vol_value);
				TFA_SET_BF(g_tfa98xx->handle, VOLSEC, tfa98xx_vol_value);
				tfa98xx_vol_value = 0;
			}
			/* EB1065A End   AUD_18_0035 */
...
static int tfa98xx_probe(struct snd_soc_component *component)
{
	...
	ret = snd_soc_codec_set_cache_io(component, 8, 16, SND_SOC_REGMAP);
	if (ret != 0) {
...
static void __tfa98xx_irq(struct tfa98xx *tfa98xx)
{
	...
	if (!(val & ie)) {
		 ...
		 * disabled
		 */
		ireg =  snd_soc_component_read32(tfa98xx->component, TFA98XX_INTERRUPT_REG);
