...
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	static struct nxpTfaVolumeStepRegisterInfo *p_regInfo = NULL;
...
enum Tfa98xx_Error tfaContWriteRegsProf(int dev_idx, int prof_idx) {
	...
	for(i=0;i<prof->length;i++) {
		...
		if  ( prof->list[i].type == dscRegister ) {
			err = tfaRunWriteRegister( dev_idx, (nxpTfaRegpatch_t *)( prof->list[i].offset+(char*)g_cont));
		}
		if ( err ) break;
	}
	return err;
...
enum Tfa98xx_Error tfaContWriteProfile(int dev_idx, int prof_idx, int vstep_idx, int load_newsettings) {
#else
...
if (load_newsettings) {
	...
	if ((prof->group != previous_prof->group || prof->group == 0) && (2 == dev_family)) {
		if (TFA_GET_BF(dev_idx, REFCKSEL) == 0) {
