...
#if 1
static bool tfa98xx_calibration_flag = false;
#endif
        ...
        if ( !(TFA_GET_BF(handle, ACS)))
        {
            switch (TFA_GET_BF(handle, REV) & 0xff)
            {
                ...
                case 0x97:
                    ...
                    pr_info("Reset 97/96 at first cold start up\n");
                    break;
                default:
                    break;
            }
            /* SL: when device is warm, enable partial update */
...
#if 1
        if( strcmp(tfaContProfileName(handle, profile), "Handset") == 0) {
            pr_debug("%s CFE is skipped to set 1 !", __func__);
        } else {
...
enum Tfa98xx_Error tfaRunSpeakerStartup(Tfa98xx_handle_t handle, int force, int profile)
{
	...
	if ( !force ) { // in case of force CF already runnning
		...
		if (tfa98xx_calibration_flag) {
			pr_debug("%s: coldboot and set MTPOTC\n", __func__);
...
#if 1
void tfaSetAcsAndCalibrationFlag(Tfa98xx_handle_t handle)
{
	if (tfa98xx_dev_family(handle) == 2) {
	    /* restore MANSCONF and MANCOLD to POR state */
	    TFA_SET_BF_VOLATILE(handle, MANSCONF, 0);
...
//	profile=profile;
	...
	if (!tfa98xx_calibration_flag) {
		if (tfa98xx_dev_family(handle) == 2)
		{
			...
			pr_debug("%s: return Tfa98xx_Error_NoClock\n", __func__);
			return Tfa98xx_Error_NoClock;
		}
    ...
    /* SL: In kernel, there's no clock available during cold start, therefore power down chip and turn off DSP*/
	if (tfa98xx_dev_family(handle) == 2)
    {
        TFA_SET_BF_VOLATILE(handle, PWDN, 1);
        TFA_SET_BF_VOLATILE(handle, SBSL, 1);
...
enum Tfa98xx_Error tfaRunUnmute(Tfa98xx_handle_t handle)
{
    ...
    if(tfa98xx_dev_family(handle) == 2) {
        ...
        if ((TFA_GET_BF(handle, CFE) != 0) && (manstate == 9)) {
            /* handset mode */
...
#ifdef TFA9888_OPTIMIZED_SWITCH_PROFILE_SEQUENCE_FTR
static int profile_before_standby = -1;
#endif
...
#if 1
		if (tfa98xx_calibration_flag) {
			pr_debug("%s: calibration_flag is cleared\n", __func__);
			tfa98xx_calibration_flag = false;
		}
#endif
...
#ifdef TFA9888_OPTIMIZED_SWITCH_PROFILE_SEQUENCE_FTR
			/* if current profile is not .standby, we will copy idx to latest_profile_idx */
			if (next_profile != profile_before_standby)
				load_new_setting = 1;
            else
                load_new_setting = 0;

			err = tfaContWriteProfile(dev, next_profile, vstep[dev], load_new_setting);
#else
...
#ifdef TFA9888_OPTIMIZED_SWITCH_PROFILE_SEQUENCE_FTR
            /* SL: when next_profile is standby then save the active profile
            to before_standby*/
            profile_before_standby = active_profile;
#endif
...
enum tfa_error tfa_stop(void)
{
	...
	for( dev=0; dev < devcount; dev++) {
        ...
        if (tfa98xx_dev_family(dev) == 2)/* Max2 */
        {
            do
            {
                if(strstr(tfaContProfileName(dev, stop_profile), ".standby") != NULL) {
...
#ifdef TFA9888_OPTIMIZED_SWITCH_PROFILE_SEQUENCE_FTR
                profile_before_standby = tfa_get_swprof(dev);
			    err = tfaContWriteProfile(dev, stop_profile, vstep[dev], 1);
#else
