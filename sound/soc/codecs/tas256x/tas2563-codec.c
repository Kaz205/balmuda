...
#define TAS2563_CAL_NAME    "/data/vendor/audio/tas2563_cal.bin"
...
#define TAS2563_IVSENSER_ENABLE  1
#define TAS2563_IVSENSER_DISABLE 0
...
static int tas2563_slot_config(void *codec, struct tas2563_priv *pTAS2563, int blr_clk_ratio)
{
	int ret = 0;
	...
	struct snd_ctl_elem_value *pValue)
{
	...
	if( nProgram == 0 ){
		/* "TAS2563 Configuration" value="0" */
		mutex_lock(&pTAS2563->codec_lock);
		ret = tas2563_set_config(pTAS2563, 0);
		mutex_unlock(&pTAS2563->codec_lock);

		/* "TAS2563 VBoost Ctrl" value="init" */
		pTAS2563->update_bits(pTAS2563, TAS2563_PowerControl,
			TAS2563_PowerControl_ISNSPower_Mask |
			TAS2563_PowerControl_VSNSPower_Mask,
			TAS2563_PowerControl_VSNSPower_PoweredDown |
			TAS2563_PowerControl_ISNSPower_PoweredDown);
		pTAS2563->update_bits(pTAS2563, TAS2563_PlaybackConfigurationReg0,
			TAS2563_PlaybackConfigurationReg0_AmplifierLevel40_Mask,
			TAS2563_PlaybackConfigurationReg0_AmplifierLevel40_8_0dBV);
		pTAS2563->update_bits(pTAS2563, TAS2563_BrownOutPreventionReg0,
			TAS2563_BrownOutPreventionReg0_BOPEN_Mask,
			TAS2563_BrownOutPreventionReg0_BOPEN_Disabled);
		pTAS2563->update_bits(pTAS2563, TAS2563_BoostConfiguration,
			TAS2563_BoostConfiguration_BoostMode_Mask |
			TAS2563_BoostConfiguration_BoostEnable_Mask,
			TAS2563_BoostConfiguration_BoostMode_AlwaysOff |
			TAS2563_BoostConfiguration_BoostEnable_Disable);
	}
	return ret;
}
...
static int tas2563_iv_enable(struct tas2563_priv *pTAS2563, int enable)
{
	int n_result;

	if (enable) {
		pr_debug("%s:tas2563iv_enable\n", __func__);
		n_result = pTAS2563->update_bits(pTAS2563,
	...
	tas2563iv_enable = enable;

	return n_result;

}

static int tas2563iv_put(struct snd_kcontrol *pKcontrol,
				struct snd_ctl_elem_value *pValue)
{
#ifdef KCONTROL_CODEC
...
#endif
	struct tas2563_priv *pTAS2563 = snd_soc_component_get_drvdata(component);
	int iv_enable = 0, n_result = 0;
	int ret, value;

	mutex_lock(&pTAS2563->codec_lock);

	iv_enable = pValue->value.integer.value[0];

	n_result = tas2563_iv_enable(pTAS2563, iv_enable);

	pr_debug("%s: tas2563iv_enable = %d\n", __func__, tas2563iv_enable);


	ret = pTAS2563->read(pTAS2563, TAS2563_PowerControl, &value);
	pr_debug("%s: PowerControl = %x\n", __func__, value);

	mutex_unlock(&pTAS2563->codec_lock);
	return n_result;
}

static int tas2563iv_get(struct snd_kcontrol *pKcontrol,
				  struct snd_ctl_elem_value *pValue)
{
#ifdef KCONTROL_CODEC
	struct snd_soc_component *component = snd_soc_kcontrol_component(pKcontrol);
	...
	ret = pTAS2563->read(pTAS2563, TAS2563_PowerControl, &value);
	if (ret < 0)
		dev_err(pTAS2563->dev, "can't get ivsensor state %s, L=%d\n",
			__func__, __LINE__);
	else if (((value & TAS2563_PowerControl_ISNSPower_Mask)
			== TAS2563_PowerControl_ISNSPower_Active)
			&& ((value & TAS2563_PowerControl_VSNSPower_Mask)
			== TAS2563_PowerControl_VSNSPower_Active)) {
		pValue->value.integer.value[0] = TAS2563_IVSENSER_ENABLE;
	} else {
		pValue->value.integer.value[0] = TAS2563_IVSENSER_DISABLE;
	}

	tas2563iv_enable = pValue->value.integer.value[0];
	dev_info(pTAS2563->dev, "value: 0x%x, tas2563iv_enable %d\n",
			value, tas2563iv_enable);
...
static int tas2563_set_volume(struct snd_kcontrol *pKcontrol,
				struct snd_ctl_elem_value *pValue)
{
#ifdef KCONTROL_CODEC
	struct snd_soc_component *component = snd_soc_kcontrol_component(pKcontrol);
	...
	unsigned int nData3 = 0;
	unsigned int nData4 = 0;

	mutex_lock(&pTAS2563->codec_lock);

	tbl_index = pValue->value.integer.value[0];
	tmp = tas_gaintable[tbl_index];
	for (i = 0; i < 4; i++)
		block[i] = (tmp >> ((3 - i) * 8));

	if(tbl_index < TAS2563_Gain_Min || TAS2563_Gain_Max < tbl_index){
		dev_err(pTAS2563->dev, "%s: Set a value outside the range(tbl_index=%d)\n",
			__func__, tbl_index);
	...
	nResult = pTAS2563->bulk_write(pTAS2563, TAS2563_PCM_Degital_Volume_Control, block, 4);

	nResult = pTAS2563->read(pTAS2563, TAS2563_REG(0x00, 0x02, 0x0C), &nData1);
	nResult = pTAS2563->read(pTAS2563, TAS2563_REG(0x00, 0x02, 0x0D), &nData2);
	nResult = pTAS2563->read(pTAS2563, TAS2563_REG(0x00, 0x02, 0x0E), &nData3);
	nResult = pTAS2563->read(pTAS2563, TAS2563_REG(0x00, 0x02, 0x0F), &nData4);
	dev_info(pTAS2563->dev, "%s: Set Register : %02X %02X %02X %02X , index=%d\n", __func__, nData1,nData2,nData3,nData4,tbl_index);
...
static int tas2563_get_volume(struct snd_kcontrol *pKcontrol,
				struct snd_ctl_elem_value *pValue)
{
#ifdef KCONTROL_CODEC
	struct snd_soc_component *component = snd_soc_kcontrol_component(pKcontrol);
	...
	nResult = pTAS2563->bulk_read(pTAS2563, TAS2563_PCM_Degital_Volume_Control, block, 4);
	get_value = ((int)block[0] << 24)
			|   ((int)block[1] << 16)
			|   ((int)block[2] << 8)
			|   (int)block[3];

	dev_info(pTAS2563->dev, "%s: Bulk Read Register : %08X\n", __func__, get_value);


	for(tbl_index = 0; tbl_index <= TAS2563_Gain_Max; tbl_index++) {
		if(tas_gaintable[tbl_index] == get_value)
			break;
	}

	if(tbl_index > TAS2563_Gain_Max){
		dev_warn(pTAS2563->dev, "%s: Get GAIN value out of range, tbl_index=%d\n",
			__func__, tbl_index);
	}
	pValue->value.integer.value[0] = tbl_index;
...
static void tas2563_codec_remove(struct snd_soc_component *component)
{
	struct tas2563_priv *pTAS2563 = snd_soc_component_get_drvdata(component);
...
//			TAS2563_BoostConfiguration_BoostMode_ClassH);
	...
	dev_info(pTAS2563->dev, "%s, vboost_state:%d\n", __func__,vboost_state);
	...
	default:
		...
		pTAS2563->update_bits(pTAS2563, TAS2563_PlaybackConfigurationReg0,
			TAS2563_PlaybackConfigurationReg0_AmplifierLevel40_Mask,
			TAS2563_PlaybackConfigurationReg0_AmplifierLevel40_16_0dBV);
		pTAS2563->update_bits(pTAS2563, TAS2563_BrownOutPreventionReg0,
			TAS2563_BrownOutPreventionReg0_BOPEN_Mask |
			TAS2563_BrownOutPreventionReg0_BOSDEN_Mask,
			TAS2563_BrownOutPreventionReg0_BOPEN_Enabled |
			TAS2563_BrownOutPreventionReg0_BOSDEN_Disabled);
		pTAS2563->update_bits(pTAS2563, TAS2563_BoostConfiguration,
			TAS2563_BoostConfiguration_BoostMode_Mask |
			TAS2563_BoostConfiguration_BoostEnable_Mask,
			TAS2563_BoostConfiguration_BoostMode_ClassH |
			TAS2563_BoostConfiguration_BoostEnable_Enable);
		break;
	}
...
static const struct snd_kcontrol_new tas2563_snd_controls[] = {
	SOC_SINGLE_TLV("TAS2563 Amp Output Level", TAS2563_PlaybackConfigurationReg0,
		0, 0x16, 0,
		tas2563_digital_tlv),
	SOC_SINGLE_EXT("TAS2563 Program", SND_SOC_NOPM, 0, 0x00FF, 0, tas2563_program_get,
		tas2563_program_put),
	SOC_SINGLE_EXT("TAS2563 Configuration", SND_SOC_NOPM, 0, 0x00FF, 0,
		tas2563_configuration_get, tas2563_configuration_put),
	SOC_SINGLE_EXT("TAS2563 Calibration", SND_SOC_NOPM, 0, 0x00FF, 0,
		tas2563_calibration_get, tas2563_calibration_put),
	SOC_ENUM_EXT("TAS2563 VBoost Ctrl", vboost_ctl_enum[0],
		tas2563_vboost_ctl_get, tas2563_vboost_ctl_put),
	SOC_ENUM_EXT("TAS2563 IVSENSE ENABLE", tas2563_enum[0],
		tas2563iv_get, tas2563iv_put),
	SOC_SINGLE_EXT("TAS2563 Digital Volume", SND_SOC_NOPM, 0, 0x003C, 0,
		tas2563_get_volume, tas2563_set_volume),
};
